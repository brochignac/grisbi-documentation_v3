...
% $ Id: 02-grisbi-manual-intro-en.tex, v 3.0 2025/01 Dominique Brochard
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Grisbi is software in active development, any feedback (idea, bug, documentation\dots{}) is welcome. You can send it to the relevant lists referenced in the  \vref{introduction-contacts} \menus{Contacts} section or on the  \lang{Grisbi}\footnote{\urlGrisbi{}} site.
...
\begin{itemize}
	...
	\item \keys{{+}} (\keys{{-}}) in a date field increment (decrement) the date%Les touches \key{+} (\key{-}) dans un champ de date incrémentent (décrémentent) la date
...
\begin{itemize}
	...
	\item Harmonisation of date fields:%Harmonisation dans les champs de date:
		\begin{itemize}
			\item[\textopenbullet] \keys{Ctrl+Enter \return} in a date field opens a calendar%\key{Ctrl}\key{Entrée} dans un champ de date ouvre un calendrier
			\item[\textopenbullet] the arrow keys are active in a calendar%les touches fléchées sont actives dans un calendrier
			\item[\textopenbullet] \keys{Ctrl+{+}} (\keys{Ctrl+{-}}) in a date field increases (decreases) the date by approximately one week%\key{Ctrl}\key{+} (\key{-}) dans un champ de date augmente (diminue) la date d'environ une semaine
			\item[\textopenbullet] \keys{Pg.Up} (\keys{Pg.Dn}) increases (decreases) the date by approximately one month%\key{Pg.Préc} (\key{Pg.Suiv}) augmente (diminue) la date d'environ un mois
			\item[\textopenbullet] \keys{Ctrl+Pg.Up} (\keys{Ctrl+Pg.Dn}) increases (decreases) the date by approximately one year%\key{Ctrl}\key{Pg.Préc} (\key{Pg.Suiv}) augmente (diminue) la date d'environ un an
		\end{itemize}	
\end{itemize}
...
\begin{itemize}
	...
	\item Global configuration in \gls{XML}\index{XML} (thanks to Axel \familyname{Rousseau})
...
\begin{itemize}%\itemsep=-3pt
	...
	\item Full-screen display command by function key \keys{F11}
	\item Keyboard shortcut \keys{Ctrl+T} for the call of a new operation
	\item Direct access to the user manual through the menu \menus{Help} or the keyboard shortcut \keys{Ctrl+H}
\end{itemize}
...
%If you want to regularly compile the latest version of Grisbi's code from \gls{Git}, you will find it advantageous to subscribe to the \lang{Git information list}\footnote{\urlListCVSEmail{}} to be notified of the latest \cmds{commit}.
...
%To subscribe to one of these lists, simply go to the page \urlListSF{}\emph{list} replacing \emph{list} with the name of the list you are interested in. Or go to \lang{Grisbi}\footnote{\urlGrisbi{}} and look for \cmds{Contacts} there.
To subscribe to one of these lists, simply go to the \urlListDiffGrisbi{} page and click on the list(s) you are interested in.

You can also use the discussion forums (or newsgroups) with a program called ``news reader'' (like Thunderbird for example) by entering \cmds{listes.grisbi.org} as the group server name.
...
A big thank you also to all the contributors on the development list who helped Grisbi's evolution through their suggestions, remarks and bug reports, as well as to the many readers of the \menus{User Manual}, which contribute to make it a better tool.
...
\Note{}: this version 3.0 of the manual is a mixture of an update of the English version 1.0 and a translation of the French version 3.0.

\vspacepdf{5mm}

This manual was written using the \gls{text editor} \gls{LaTeX}, and is available in \gls{file format} \indexword{\gls{PDF}}\index{PDF} or \indexword{\gls{HTML}}\index{HTML}, with illustrations (screenshots) in both formats.
...
All these different formats can also be downloaded from the \lang{Sourceforge}\footnote{\urlSourceForgeDocumentation{}} site, as well as the corresponding versions of the software on \lang{Sourceforge}\footnote{\urlSourceForge{}} in the ``\textsf{grisbi stable}'' or ``\textsf{grisbi unstable}'' folders.

\vspacepdf{5mm}

The tools needed to read the various manual formats are presented in the  \vref{introduction-manual-readers} section \menus{Reading software}.
...
\begin{itemize}
	\item \menus{Preamble} Introduces the English translation and explains the origin of the name given to this software;
	\item \menus{Introduction} Introduces the software, the manual, their authors and contacts;
	\item \menus{First start Grisbi} is the \emph{essential} chapter to help you to start using the software;
	\item \menus{Home} describes the main elements of the GUI and their manipulation with the mouse and the keyboard (shortcuts);
	\item \menus{Export and Import Accounts} describes how to exchange data with other software;
	\item \menus{Data Management} presents the options of the account files, backups and archives and their management;
	\item \menus{Accounts Management} describes the properties of accounts, their management and the different types of accounts with their use;
	\item \menus{Account Operations}  describes how transactions are handled, the information and input fields used and how they are managed, and how transactions are created and managed;
	\item \menus{Bank Reconciliation} details the procedure for reconciling an account and managing reconciliations;
	\item \menus{Schedule} describes the planning of future entries and their manipulation;
	\item \menus{Searches} takes stock of data search possibilities;
	\item \menus{Payees}, \menus{Categories}, \menus{budgetary allocations} and \menus{Exercises} describe the management of this data;
	\item \menus{Credit Simulation} describes methods and simulation management;
	\item \menus{Forecast Budgets} describes the tools and procedures for budget creation and depreciation tables, as well as their management;
	\item \menus{Credit card management and their prediction} describes the management of these cards, including deferred debit cards, and forecasting methods;
	\item \menus{Business Accounting} presents two introductions for business managers (for French only)
	\item \menus{Reports} and \menus{Report creation} describe the management and creation of reports;
	\item \menus{Configuration of Grisbi} details all the possibilities of setting the software;
	\item \menus{Maintenance Tools} gives some tips to use in case of errors or bugs.
\end{itemize}
...
% END_TODO
	\item \Note{}: underlines a particular point to be taken into account
	\item \Attention{}: indicates either a point that is very important for understanding, or an error that must not be made on pain of serious damage to your data;\newline
	\Attention{} indicates a point that \emph{must be respected}.
...
\begin{itemize}
	\item for the \indexword{\gls{PDF}}\index{PDF} format:
		\begin{itemize}
	...
	\item for the \indexword{\gls{HTML}}\index{HTML} format:
		\begin{itemize}
