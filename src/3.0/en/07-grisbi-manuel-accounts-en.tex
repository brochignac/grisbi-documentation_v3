...
% espace pour changement de thème
\vspacepdf{5mm}
The navigation panel displays the name of the selected account on a {\setlength{\fboxsep}{2pt}\colorbox[RGB]{235,149,149}{dark pink background}}; the information bar displays the account name on the left and the account balance on the far right; the details panel displays two tabs by default:
...
\begin{enumerate}
	...
	\end{figure}
	% image centrée
	\item the new account is then created and selected in the navigation panel: its name appears on a {\setlength{\fboxsep}{2pt}\colorbox[RGB]{235,149,149}{dark pink background}}, and the details panel displays the \menus{Properties} tab, where you can modify or complete the settings; the \menus{Transactions} tab is, of course, empty of any transactions.
...
\noindent
\underline{\textbf{Translation note}}: the following sections are translations from the French manual and are not transposed into Anglo-Saxon accounting. Furthermore, the author does not have Anglo-Saxon linguistic and accounting skills. Therefore, the accuracy of the examples and terms used cannot be guaranteed.
\newline
A proofreader would be most welcome.
%Note de traduction: les sections suivantes sont la traduction du manuel français et non la transposition vers la comptabilité anglo-saxonne. De plus, l'auteur n'a pas les compétences linguistiques et comptables anglo-saxonnes. Donc l'exactitude des exemples et des termes employés n'est pas garantie.
...
\begin{tblr}{
		%width=0.95\textwidth,
		%colsep=4pt,							%horizontal space on both sides of columns, default=6
		colspec={Q[colsep=3pt,c] Q[colsep=3pt,2.8cm,c] Q[colsep=3pt,1.9cm,c] Q[3.9cm,c] Q[3.8cm,c]},
		%colspec={Q[1cm,r] Q[1.5cm,c] Q[1cm,l]},
		%rowspec={Q[m] Q[m] Q[m] Q[m] Q[m]},
		%rowsep=4.5pt,
		%leftsep=4pt,							%default=6
...
\begin{tblr}{
		%width=0.95\textwidth,
		%colsep=4pt,							%horizontal space on both sides of columns, default=6
		colspec={Q[colsep=3pt,c] Q[4.4cm,c] Q[c] Q[6.4cm,c]},
		%colspec={Q[1cm,r] Q[1.5cm,c] Q[1cm,l]},
		%rowspec={Q[m] Q[m] Q[m] Q[m] Q[m]},
		%rowsep=4.5pt,
		%leftsep=4pt,							%default=6
	...
	\menus{Date} & \menus{Payee}	& \menus{Debit} & \menus{Categories}		 \\
	06/01/2025 	 & Car depreciation	& £5,000.00	    &  Depreciation: Devaluation \\
\end{tblr}
