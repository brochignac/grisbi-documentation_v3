...
\end{figure}
	...
	\item Enter details of \indexword{banks}\index{banks!definition} holding your accounts (step 5/6):
		\begin{enumerate} 
 			\item click \menu{Add} to define a bank; fill in the details of the bank (name, bank code, etc.),
			\item select a bank from the list and click the \menu{Remove} button to delete a bank, then confirm in the window that opens,
			\item confirm with the \menu{Following} button to go to the next step, \menu{Create a new account}:
		\end{enumerate} 
...
\end{figure}
		...
		\begin{itemize}
			\item[\textopenbullet] \menu{Create a new account from scratch}:\par
			...
			\item[\textopenbullet] \menu{Import data from online bank services or from accounting software}:\par
