...
By selecting the \menu{Accounts} tab, the details panel displays:

\begin{itemize}
	\item to the top in a grey background banner:
		\begin{itemize}
			\item on the left, the \menu{Grisbi} icon (which can be hidden, see \vref{setup-display-logo-icon}),
			\item and on the right the \indexword{title}\index{title display!title}\index{home page} of the accounts file you currently have loaded, in the form \enquote{assigned name - Grisbi}; you can define this label, from among three possibilities, in the \menu{Edit - Preferences} menu (see the \vref{setup-display-addresses-titles} paragraph, Addresses \& titles):
				\begin{itemize}
	...
	\item in the main light grey area below the banner:
		\begin{itemize}
			\item for each currency separately, for all accounts and \indexword{groups of accounts}\index{group of accounts}, under the label \menu{Reconciled balance} and \menu{Current balance}:
				\begin{itemize}
				...
				\end{itemize}
			\item the \indexword{warnings from automatically scheduled entries}\index{alert!scheduled entry} with their date, wording and amount, according to the choices made in the \menu{Edit - Preferences} menu (see the section \vref{setup-general-planned}, \menu{Scheduler}),
			\item the list of accounts whose balance has fallen below the \menu{Minimum authorized balance},
			\item the list of accounts whose balance has fallen below the \menu{Minimum desired balance}.
		\end{itemize}
\end{itemize}

\vspacepdf{5mm}

\textbf{Note}: For definitions of \menu{Minimum authorized balance} and \menu{Minimum desired balance}, see the \vref{accounts-properties}, \menu{Account Properties} section.
...
A balance greater than the \menu{Minimum desired Balance} is displayed in \textcolor[RGB]{0,126,0}{green}: as the cursor moves over the line, its colour changes to \textcolor[RGB]{0,227,0}{light green}.

A balance less than the \menu{Minimum desired balance} and greater than the \menu{Minimum authorized balance} is displayed in \textcolor[RGB]{230,155,0}{orange}: as the pointer passes on its line, this color changes to \textcolor[RGB]{255,200,0}{light orange}.

A balance less than the  \menu{Minimum authorized Balance}  is displayed in \textcolor[RGB]{153,0,0}{dark red}: as the pointer passes over its line, this color changes to \textcolor{red}{red}.
...
\begin{itemize}
	\item in the \menu{Edit - Preferences} menu: %TODO verify sections/paragraphs with \vref{setup-xxx}
		\begin{itemize}
			\item \menu{\indexword{Generalities}\index{generalities}}:
				\begin{itemize}%[label=-]
					\item \menu{Various settings}, \menu{Scheduler}\index{scheduler} tab: section \vref{setup-general-planned};
					\item \menu{Main page}:
					\begin{itemize}%[label=*]
						\item \menu{Calculation of balance}: paragraph \vref{setup-general-home-balance},
						\item \menu{Balances partials of the list of accounts}: paragraph \vref{setup-general-home-partBalance};
					\end{itemize}
				\end{itemize}
			\item \menu{\indexword{Display}\index{display}}:
				\begin{itemize}[label=-]
					\item \menu{Fonts \& Logo}\index{fonts}\index{logo}: section \vref{setup-display-logo},
					\item \menu{Addresses \& titles}\index{title}\index{addresses}: section \vref{setup-display-addresses-titles};
				\end{itemize}
	 	\end{itemize}
 	\item in the \menu{Properties} tab of each account in the \menu{Balances} section:
	 	\begin{itemize}
	 		\item Accounts below the \menu{Minimum authorised balance}\index{balance!minimum authorised}: section \vref{accounts-properties}:
	 		\item Accounts below the \menu{Minimum desired Balance}\index{balance!minimum desired}: section  \vref{accounts-properties}.
	 	\end{itemize}
\end{itemize}
