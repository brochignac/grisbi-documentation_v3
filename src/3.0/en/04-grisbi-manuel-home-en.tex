...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menus{Edit transaction}: allows an existing operation to be rectified, see section \vref{transactions-modify}, \menus{Modification d'une opération};%TODO to translate
		...
		\item \menus{Edit transaction}: allows the creation of a new transaction in an account (shortcut \keys{Ctrl+T}), see the section \vref{transactions-new}, \menus{Saisie d'une nouvelle opération};%TODO to translate
		...
		\item \menus{Remove transaction}: deletes an existing operation, see section \vref{transactions-delete}, \menus{Deleting a transaction};
