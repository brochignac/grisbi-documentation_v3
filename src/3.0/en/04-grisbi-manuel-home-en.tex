...
\Note{}: buttons with the names of account files that Grisbi has already used are only present if these files exist; if you want to remove them from this entry page, move them to another directory, or delete them.%les pavés portant les noms des fichiers de comptes que Grisbi a déjà utilisés ne sont présents que si ces fichiers existent; si vous voulez les enlever de cette page d'entrée, déplacez-les dans un autre répertoire, ou supprimez-les.
...
\Note{}: the information bar, displayed by default, can be hidden by unchecking a box in the preferences \vref{setup-display-toolbars}.
...
\Note{}: Depending on the theme of the desktop environment or window manager you are using, these triangular symbols might be replaced by other characters such as +, -, >, <, etc.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\newline
			\Note{}: you can adjust the display order of the partial balances of the account groups (see the section \vref{setup-general-home-partBalance}, \menus{Partial balances of the list of accounts}),		 
...
\Note{}: For definitions of \menus{Minimum authorized balance} and \menus{Minimum desired balance}, see the \vref{accounts-properties}, \menus{Account Properties} section.
...
\begin{itemize}
	\item in the \menus{Edit - Preferences} menu:
	\begin{itemize}
...
\Note{}: in the \menus{Edit} menu, some entries are only active when an account or transaction is selected.
...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menus{Edit transaction}: allows a selected transaction to be rectified, see section \vref{transactions-modify}, \menus{Editing a transaction};
		...
		\item \menus{New transaction}: allows the creation of a new transaction in an account (shortcut \keys{Ctrl+T}), see the section \vref{transactions-new}, \menus{Entering a new transaction};
...
\Note{}: in the \menus{View} menu, entries are only active when an account is selected.
...
\begin{minipage}{.7\linewidth}
	...
	\item \menus{Show \indexword{closed accounts}}\index{closed!account}: displays account(s) that have been closed and not deleted, see section \vref{accounts-properties}, \menus{Account properties};
...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menus{User's Manual}: opens the \enquote{Grisbi Manual} in your browser or in your \gls{PDF}\index{PDF} reader according to your choice (see \vref{setup-general-various-general-display}, \menus{Help display})(shortcut \keys{F1}); 
...
\subsection{List of transactions or scheduled transactions}%Liste des opérations ou des opérations planifiées

\begin{itemize}
	\item Move selection: \keys{\arrowkeyup} or \keys{\arrowkeydown}%Déplacer la sélection: \keys{\arrowkeyup} ou \keys{\arrowkeydown}
	\item Select a transaction: \keys{Enter \return}%Sélectionner une opération: \keys{Entrée \return}
	\item New transaction:%Nouvelle opération:
	\begin{itemize}
		\item On an empty line: \keys{Enter \return}%Sur une ligne vide: \keys{Entrée \return}
		\item On an existing transaction: \keys{Ctrl+T}%Sur une opération existante: \keys{Ctrl+T}
	\end{itemize}
	\item Modify a transaction: \keys{Enter \return}%Modifier une opération: \keys{Entrée \return}
	\item Delete a transaction: \keys{Delete}%Supprimer une opération: \keys{Suppr}
	\item Point ou unpoint a transaction for a reconciliation:%Pointer ou dépointer une opération:
		\begin{itemize}
			\item in the reconciliation mode: \keys{Ctrl+P}, \keys{F12} ou \keys{Space},
			%dans le mode rapprochement: \keys{Ctrl+P} ou \keys{F12} ou \keys{Espace},
			\item out of reconciliation mode: \keys{Ctrl+P} or \keys{F12}
			%hors mode rapprochement: \keys{Ctrl+P} ou \keys{F12}
		\end{itemize}
	\item Reconcile or un-reconcile a transaction: \keys{Ctrl+R}\newline
	...
	\item Roll out/in a splitted transaction: \keys{Space}%Déplier/replier une opération ventilée: \keys{Espace}
	\item Show or hide archival lines: \keys{Alt+L}%Montrer ou masquer les lignes d'archives: \keys{Alt+L}
	\item In the list of account transactions:%Dans la liste des opérations d'un compte:
	\begin{itemize}
		\item Open the character search window: \keys{Ctrl+F}%Ouvrir la fenêtre de recherche de caractères: \keys{Ctrl+F}
	\end{itemize}
\end{itemize}


\subsection{Transaction/Scheduled form}%Formulaire de saisie des opérations

\begin{itemize}
	\item The \keys{Enter \return} key is configurable (see preferences \vref{setup-form-behaviour-enter}): it can be set to either move in the input form, or to validate the entry;
	...
	\item Accept auto-complete: \keys{Tab \tab} ou \keys{Enter \return} (depending on your configuration choice);
...
\subsection{Drop down lists}%Listes déroulantes
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Move in the list: \keys{\arrowkeyup} or \keys{\arrowkeydown}%Se déplacer dans la liste: \keys{\arrowkeyup} ou \keys{\arrowkeydown}
		\item Validate the item in the list: \keys{\Space{Space}\Space}%Valider l'item de la liste: \keys{\Space{Espace}\Space}
	\end{itemize}
\end{itemize}

\subsection{Dates entered on the calendar}%Dates saisies au calendrier

\begin{itemize}
	\item Open a monthly calendar (on the date field): \keys{Ctrl+Enter \return}%Ouvrir un calendrier mensuel (sur le champ de date): \keys{Ctrl+Entrée \return}
	\item Closes the calendar without changing the date: \keys{Esc}%Fermer le calendrier sans modifier la date: \keys{Ech}
	\item Validate the selected date: \keys{Enter \return}%Valider la date sélectionnée: \keys{Entrée \return}
	\item Next or previous day: \keys{{+}} or \keys{{-}}, \keys{\arrowkeyright} or \keys{\arrowkeyleft}%Jour suivant ou précédent: \keys{{+}} ou \keys{{-}}, \keys{\arrowkeyright} ou \keys{\arrowkeyleft}
	\item Previous or next week: \keys{\arrowkeyup} or \keys{\arrowkeydown}%Semaine précédente ou suivante: \keys{\arrowkeyup} ou \keys{\arrowkeydown}
	\item Previous or next month: \keys{PageUp} or \keys{PageDown}%Mois précédent ou suivant: \keys{Pg.Préc} ou \keys{Pg.Suiv}
	\item First day or last day of the month: \keys{Home / \,$\nwarrow$\,} or \keys{End}%Premier jour ou dernier jour du mois: \keys{Début / Orig / \,$\nwarrow$\,} ou \keys{Fin}
\end{itemize}

\subsection{Dates entered by keyboard}%Dates saisies au clavier

\begin{itemize}
	\item Next or previous day: \keys{{+}} or \keys{{-}}%Jour suivant ou précédent: \keys{{+}} ou \keys{{-}}
	\item Previous or next week: \keys{Ctrl+{+}} or \keys{Ctrl+{-}}%Semaine précédente ou suivante: \keys{Ctrl+{+}} ou \keys{Ctrl+{-}}
	\item Previous or next month: \keys{PageDown} or \keys{PageUp}%Mois précédent ou suivant: \keys{Pg.Suiv} ou \keys{Pg.Préc}
	\item Previous or Next Year: \keys{Ctrl+PageDown} or \keys{Ctrl+PageUp}%Année précédente ou suivante: \keys{Ctrl+Pg.Suiv} ou \keys{Ctrl+Pg.Préc}
	\item Validate the selected date: \keys{Enter \return}%Valide la date sélectionnée \keys{Entrée \return}
\end{itemize}

\subsection{Payees, categories, budgetary lines, credits simulator, historical data and forecast}
			%Tiers, catégories, imputations budgétaires, simulateur de crédits, données historiques et prévisions

\begin{itemize}
	\item Move selection: \keys{\arrowkeyup}, \keys{\arrowkeydown}, \keys{PageUp} or \keys{PageDown}
	...
	\item Display the transactions of sub-categories or sub-budgetary lines (on a sub-category or a sub-budgetary line): \keys{Enter \return}
...
\begin{itemize}
	\item Show the Grisbi preferences window: \keys{Ctrl+Shift \shift+P}
...
\begin{itemize}
	\item Open the Grisbi Manual: \keys{F1}
