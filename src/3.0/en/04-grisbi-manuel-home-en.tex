...
%TODO following
\begin{itemize}
	 \item in a box on a gray background, the \menu{Grisbi} icon (which can be hidden \vref{setup-display-logo-icon}) on the left and on the right the \indexword{title}\index{title display!title}\index{home page !page d'accueil} of the accounts file you currently have loaded, in the form \enquote{assigned name - Grisbi}; ??you can define this name in one of three ways the \menu{Edit preferences} menu (see the paragraph \vref{setup-display-addresses-titles}, \menu{Titles})??: this can be useful if you manage multiple accounting entities:
