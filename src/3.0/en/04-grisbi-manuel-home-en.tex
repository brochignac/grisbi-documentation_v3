...
\begin{itemize}[rightmargin=.6cm]
	\item \menus{New window}: non-functional (perhaps in the future?) %non fonctionnel (dans le futur peut-être?);	%TODO: to update
\end{itemize}
...
%TODO following
\begin{itemize}
	\item \menus{New account file...}: creates a new account file; the current file is closed and a new empty file is created with an empty account (shortcut \keys{Ctrl+N}),  see the \vref{start-newfile} section: not to be confused with the creation of a new account;
	\item \menus{Open...}: open an accounts file  (shortcut  \keys{Ctrl+O}):
	\item \menus{Recently opened files}: displays the list of the last n files opened with Grisbi (only if there have been several); this number is configurable in the \menus{Edit - Preferences}, see the \vref{setup-general-files-manage}, \menus{Account file management} section:
	\item \menus{Save}: Saves the current account file  (shortcut \keys{Ctrl+S}):
