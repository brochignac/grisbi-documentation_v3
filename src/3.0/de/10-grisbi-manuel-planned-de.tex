...
\vspace{2mm}
Ein \texttt{Rechtsklick} auf die Zeile eines geplanten Buchung öffnet ein Kontextmenü mit den folgenden Funktionen\refimage{planned-transactions-contextMenu-img}:
%A \texttt{right-click} on the row of a scheduled transaction displays a context menu with the following functions\refimage{planned-transactions-contextMenu-img}:
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item indem Sie mit der \texttt{linken Maustaste} auf das Werkzeug \menus{Bearbeiten} in der Symbolleiste klicken,
		%\item by \texttt{left-clicking} on the \menus{Edit} tool in the toolbar,
		%\item par \texttt{clic-gauche} sur l'outil \menus{Éditer} de la barre d'outils,
		\item indem Sie mit der \texttt{rechten Maustaste} auf die Buchungszeile klicken und dann im Kontextmenü \menus{Buchung bearbeiten} auswählen;
		%\item by \texttt{right-clicking} on the transaction line and then selecting \menus{Edit transaction} from the context menu;
...
\begin{enumerate}
	...
	\begin{itemize}
		\item entweder mit der \texttt{linken Maustaste} auf das Werkzeug \menus{Ausführen} in der Symbolleiste klicken,
		%\item either \texttt{left-clicking} on the \menus{Execute} tool in the toolbar,
		%\item soit un \texttt{clic-gauche} sur l'outil \menus{Exécuter} de la barre d'outils,
		\item oder mit der \texttt{rechten Maustaste} auf die Buchungszeile klicken und im Kontextmenü \menus{Buchung ausführen} auswählen;
		%\item or \texttt{right-clicking} on the transaction line and selecting \menus{Execute} from the context menu;
...
\begin{enumerate}
	...
	\end{figure}
	\begin{itemize}
		\item \keys{Gesamte buchung}: Alle Buchungen, sowohl aktuelle als auch zukünftige, werden gelöscht; vergangene Buchungen bleiben erhalten,
		%\item \keys{All the occurencies}: all transactions, both current and future, will be deleted; past transactions are retained,
		...
		\item \keys{Nur diese Buchung}: Das heißt, nur die aktuelle Instanz (nur die ausgewählte Buchung wird gelöscht);
		%\item \keys{Only this one}: that is, only the current occurence (only the selected transaction will be deleted);
