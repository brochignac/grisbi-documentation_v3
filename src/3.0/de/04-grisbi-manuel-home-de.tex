...
% $Id: 04-grisbi-manuel-home-fr.tex, v 3.0 2025/03 Dominique Brochard
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{itemize}
	\item in the \menus{Bearbeiten - Einstellungen} menu:
	\begin{itemize}
...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		...
		\item \menus{Zuletzt geöffnete Dateien}: zeigt eine Liste der letzten n mit Grisbi geöffneten Dateien an (nur wenn mehr als eine geöffnet wurde); diese Anzahl ist im Menü \menus{Bearbeiten - Einstellungen} konfigurierbar, siehe Abschnitt \vref{setup-general-files-manage}, {Dateiverwaltung};
...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menus{Buchung bearbeiten}: ermöglicht es, einen ausgewählten Vorgang zu korrigieren, siehe Abschnitt \vref{transactions-modify}, \menus{Eine Buchung bearbeiten};
		% allows a selected operation to be rectified, see section \vref{transactions-modify}, \menus{Editing a transaction};
		...
		\item \menus{Buchung erstellen}: ermöglicht die Erstellung einer neuen Transaktion in einem Konto (Tastenkombination \keys{Strg+T}), siehe den Abschnitt \vref{transactions-new}, \menus{Ein neue Buchung einstellen};
		% allows the creation of a new transaction in an account (shortcut \keys{Ctrl+T}), see the section \vref{transactions-new}, \menus{Entering a new transaction};
		...
		\item \menus{Buchung löschen}: löscht einen ausgewählten Vorgang, siehe Abschnitt \vref{transactions-delete}, \menus{Eine Buchung löschen};
...
\begin{itemize}
	\item \menus{Buchung als Vorlage verwenden}: Erstellt die Kopie eines ausgewählten Vorgangs, wobei das aktuelle Datum in das Buchungsformular eingetragen wird, siehe Abschnitt \vref{transactions-model}, \menus{Verwenden Buchung als Vorlage};
	%\item \menus{Use selected transaction as a template}: creates a copy of a selected transaction, with the current date entered in the transaction form, see section \vref{transactions-model}, \menus{Transaction selected as a template};
	%\item \menus{Utiliser l'opération sélectionnée comme modèle}: permet de créer une nouvelle opération à partir d'une opération sélectionnée, voir la section \vref{transactions-model}, \menus{Opération sélectionnée comme modèle};
	\item \menus{Buchung duplizieren}: Erstellt eine Kopie, die mit dem ausgewählten Buchung identisch ist, und öffnet das Buchungsformular, siehe Abschnitt \vref{transactions-duplicate}, \menus{Eine Buchung klonen};
	...
	%\item \menus{Cloner l'opération}: permet de dupliquer une opération existante, voir la section \vref{transactions-duplicate}, \menus{Clonage d'une opération};
	\item \menus{Buchung regelmäßig ausführen}: siehe Abschnitt \vref{transactions-schedule}, \menus{Umwandlung eines Buchung in eine geplante Buchung};
	%\item \menus{Convert to scheduled transaction}: see section \vref{transactions-schedule}, \menus{Converting a transaction into a scheduled transaction};
	...
	\item \menus{Buchung verschieben nach}: Verschiebt die Buchung auf das ausgewählte Konto, siehe Abschnitt \vref{transactions-move}, \menus{Verschieben eines Buchung auf ein anderes Konto};
	...
	%\item \menus{Déplacer l'opération vers un autre compte}: voir la section \vref{transactions-move}, \menus{Déplacement d'une opération vers un autre compte};
	\item \menus{Buchungen suchen} (Tastenkombination \keys{Strg+F}) (siehe Kapitel \vref{search}, \menus{Suche}):
	%\item \menus{Search} (shortcut \keys{Ctrl+F}) (see the capter \vref{search}, \menus{Search}):
	%\item \menus{Rechercher} (raccourci-clavier \keys{Ctrl+F}) (voir le chapitre \vref{search}, \menus{Recherche}):
		\begin{itemize}
			\item zeigt das Suchfenster an, nachdem Folgendes ausgewählt wurde:
			%\item displays the search box after the selection of:
			%\item affiche la fenêtre de recherche après la sélection de:
			\begin{itemize}
				\item ein Konto oder eine Buchung,
				%\item an account or a transaction,
				%\item un compte ou une opération,
				\item die Registerkarte \dequote{Empfänger} oder einen Empfänger,
				%\item the \enquote{Payees} tab or a payee,
				%\item l'onglet \frquote{Tiers} ou un tiers,
				\item die Registerkarte \dequote{Kategorien} oder eine Kategorie,
				%\item the \enquote{Categories} tab or a category,
				%\item l'onglet \frquote{Catégories} ou une catégorie,
				\item die Registerkarte \dequote{Gruppen} oder eine Gruppe.
				%\item the \enquote{Budgetary lines} tab or a budgetary line is selected
				%\item l'onglet \frquote{Imputations budgétaires} ou une imputation budgétaire,
			\end{itemize}
			\item Öffnet das Fenster \menus{Bericht bearbeiten}, wenn die Registerkarte \dequote{Berichte} im Navigationsbereich oder ein Bericht ausgewählt ist, siehe Abschnitt \vref{reports-creation}, \menus{Bericht erstellen};
			%\item opens the \menus{Report properties} window for a report when the \enquote{Reports} tab of the navigation panel or a report is selected, see chapter \vref{reports-creation}, \menus{Creating a report};
			%\item ouvre la fenêtre de \menus{Propriétés de l'état} quand l'onglet \frquote{États} du panneau de navigation ou un état est sélectionné, voir le chapitre \vref{reports-creation}, \menus{Création d'un état};
		\end{itemize}
	\item \menus{Konto erstellen}: Startet den Assistent Konto erstellen für die Erstellung eines neuen Kontos in Ihrer Grisbi-Datei (Tastenkombination \keys{Shift+Ctrl+N}), siehe Abschnitt \vref{accounts-new}, \menus{Neues Konto erstellen};
	...
	\item \menus{Konto löschen}: löscht das ausgewählte Konto aus Ihrer Grisbi-Datei, siehe Abschnitt \vref{accounts-delete}, \menus{Konto löschen};
	...
	\item \menus{Einstellungen}: ermöglicht die Konfiguration von Grisbi (Tastenkombination \keys{Shift \shift+Strg+P}); siehe Kapitel \vref{setup}, \menus{Grisbi-Konfiguration}.
...
\Note{}: Im Menü \menus{Ansicht} sind die Einträge erst aktiv wenn ein Konto ausgewählt wird.
%in the \menus{View} menu, entries are only active when an account is selected.
...
\begin{minipage}{.7\linewidth}
	...
	\item \menus{Geschlossene \indexword{Konten anzeigen}}\index{Konto!anzeigen}: zeigt Konten an, die geschlossen und nicht gelöscht wurden, siehe Abschnitt \vref{accounts-properties}, \menus{Kontodaten};
...
\begin{itemize}
	...
	\begin{itemize}
		\item Zeigt das Fenster {Buchungen Suchen} an: \keys{Ctrl+F}
		%\item Displays the \menus{Search} window: \keys{Ctrl+F}
		%\item Affiche la fenêtre \menus{Rechercher}: \keys{Ctrl+F}
	\end{itemize}
	\item Auf der Tab Berichte:
	%On the Reports tab:
	%Sur l'onglet États
	\begin{itemize}
		\item Öffnet das Fenster \menus{Bericht bearbeiten} der Suche: \keys{Ctrl+F}
		%\item Opens the \menus{Report properties} search window: \keys{Ctrl+F}
		%\item Ouvre la fenêtre des \menus{Propriétés de l'état} de recherche: \keys{Ctrl+F}
	\end{itemize}	
