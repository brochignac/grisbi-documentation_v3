...
%TODO following
\begin{itemize}
	\item \menus{New account file...}: creates a new account file; the current file is closed and a new empty file is created with an empty account (shortcut key \key{Ctrl}\key{N}),  see the \vref{start-newfile} section; not to be confused with the creation of a new account;
	\item \menus{Open...}: open an accounts file  (shortcut key  \key{Ctrl}\key{O});
