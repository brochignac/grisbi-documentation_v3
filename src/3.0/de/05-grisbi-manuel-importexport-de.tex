			...
			\item \textbf{Select options to export}:%Sélectionner les options pour l'export}:
				\begin{itemize}
					...
					% exporte le ou les comptes cochés au format \gls{CSV}; en plus des options disponibles au format QIF (ci-dessus):
						...
						\end{itemize}
				\end{itemize}
			\item \menus{Treat all files as the first}: this option exports the selected account(s) with the same name as in the list, without the possibility of modification and without choice of destination;
			%Traiter tous les fichiers comme le premier}: cette option exporte le ou les comptes sélectionnés avec le même nom que dans la liste, sans possibilité de modification et sans choix de la destination;
			\item confirm with the \menus{Following} button;
			%validez par le bouton \menus{Suivant};
		\end{itemize}
	\item For each account, define the name of the file, the destination directory and the export format; the option \menus{Treat all files as the first} bypasses this step. Then confirm with the \menus{Following} button;
	%pour chaque compte, définissez le nom du fichier, le répertoire de destination et le format d'exportation; l'option \menus{Traiter tous les fichiers comme le premier} shunte cette étape. Puis validez par le bouton \menus{Suivant}
