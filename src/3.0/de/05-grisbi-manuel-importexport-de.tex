...
\begin{enumerate}
	...
	%Sélection des fichiers de compte à importer (étape 2/4) \refimage{importexport-import-files-select-img}:
		\begin{enumerate}
			...
			\begin{figure}[htbp]
				%\centering
				\raggedleft
				\vspace{-5pt}						% space above the floating
				%\begin{flushleft}
				...
				\vspace{-20pt}						% space below the caption
	...
	\end{figure}
		\begin{itemize}
			...
			\item Wenn alles korrekt ist, bestätigen Sie den Import mit der Schaltfläche \menus{Nachfolgendes Element};
...
\section{Konten aus Grisbi exportieren\label{importexport-export}}%Export accounts from Grisbi%Export de comptes à partir de Grisbi

Wenn Sie die von Grisbi erstellten Kontodaten in einer anderen Buchhaltungsanwendung verwenden möchten, müssen Sie diese Daten zunächst in Dateien exportieren und dann mithilfe dieser Dateien in die andere Anwendung importieren. Das ausgewählte Dateiformat muss mit dem Export durch Grisbi kompatibel sein \emph{und} mit dem Import durch die Zielanwendung kompatibel sein.
...
Wählen Sie im Menü \menus{Datei} \menus{Daten exportieren} (oder verwenden Sie die Tastenkombination \keys{Strg+E}), um den Assistent Konten exportiren zu öffnen. Der Export von Konten umfasst mindestens vier Schritte:
...
\begin{enumerate}
	\item Starten des Assistenten (Stufe 1/3): Dieses Fenster zeigt an, dass alle Transaktionen in die Währung des jeweiligen Kontos umgerechnet werden, da die Dateiformate \gls{QIF} und \gls{CSV} keine Währungen unterstützen. Bestätigen Sie durch Klicken auf die Schaltfläche \menus{Nachfolgendes Element}.
	%Starting the assistant (step 1/3): this window indicates that, since the \gls{QIF} and \gls{CSV} file formats do not support currency, all transactions will be converted into the currency of their respective account; confirm with the \menus{Following} button;
	...
	\item Konten und Optionen auswählen \refimage{importexport-export-img}:
	%Selecting accounts and options \refimage{importexport-export-img}:
	%Sélection des comptes et des options
		\begin{figure}[htbp]
			\begin{center}
				\includegraphics[width=0.95\textwidth]{image/screenshot/importexport_export}
			\end{center}
			\caption{Exportieren der Konten}%Exporting accounts%Export des comptes
			\label{importexport-export-img}
		\end{figure}
		\begin{itemize}
			\item \textbf{Konten für den Export auswählen} (Stufe 2/3): Wählen Sie die zu exportierenden Konten aus, indem Sie auf das entsprechende Kästchen oder auf die Schaltfläche \menus{Alle auswählen} klicken;
			%\textbf{Select accounts to export} (step 2/3): select the accounts to export by clicking in the corresponding box or on the \menus{Select all} button;
			...
			\item \textbf{Einstellungen für den Export}%Select options to export}:%Sélectionner les options pour l'export}:
				\begin{itemize}
					\item \menus{QIF Format}: exportiert die ausgewählten Konten im Format \gls{QIF}; zusätzlich die Option:
					%exports the selected account(s) in \gls{QIF} format; in addition, the option:
					%exporte le ou les comptes cochés au format \gls{QIF}; en plus l'option:
						\begin{itemize}
							\item \menus{Amerikanisches Datumsformat verwenden}: speichert das Datum im Format \dequote{Monat/Tag/Jahr} (MM/TT/JJJJ),
							%\menus{Force US dates}: saves the date in the format \enquote{month/day/year} (mm/dd/yyyy),
							%\menus{Force les dates au format US}: enregistre la date au format \frquote{mois/jour/année} (mm/dd/yyyy),
							\item \menus{Amerikanisches Zahlenformat verwenden}: verwendet den Punkt \dequote{.} als Dezimaltrennzeichen und das Komma \dequote{,} als Tausendertrennzeichen;
							%\menus{Force US numbers}: uses the period \enquote{.} as the decimal separator and the comma \enquote{,} as the thousands separator;
						...
						\end{itemize}
					\item \menus{CSV Format}: Exportiert die ausgewählten Konten im \gls{CSV}-Format; zusätzlich zu den im QIF-Format (oben) verfügbaren Optionen:
					%exports the selected account(s) in \gls{CSV} format; in addition to the options available in QIF format (above):
					%exporte le ou les comptes cochés au format \gls{CSV}; en plus des options disponibles au format QIF (ci-dessus):
						\begin{itemize}
							\item Das Trennzeichen zwischen den Daten kann aus der Dropdown-Liste im linken Fenster ausgewählt werden und wird im rechten Fenster angezeigt, wo Sie es auch ändern können;
							%the separator between data can be selected from the drop-down list in the left window and is displayed in the right window, where you can also modify it.
							...
							\item \menus{Datum mit Hochkomma}: Wenn diese Option aktiviert ist (Standard), werden Datumsangaben wie andere Daten in Anführungszeichen gesetzt. Wenn Sie mit der Maus über das Menü fahren, öffnet sich ein Informationsfenster \dequote{Deaktivieren um das Datum nicht in Anführungszeichen zu schreiben};
							%\menus{Quote the dates}: if checked (default), dates will be enclosed in quotation marks, like other data. Hovering over the menu with the mouse opens an information window \enquote{Uncheck to write a date field without quotes};
						...
						\end{itemize}
				\end{itemize}
			\item \menus{Ausgewählte Konten automatisch bearbeiten}: Diese Option exportiert automatisch die ausgewählten Konten:
			%\menus{Automatically treat the selected account(s)}: this option automatically exports the selected account(s):
			%\menus{Traiter automatiquement les comptes sélectionnés}: cette option exporte le ou les comptes sélectionnés automatiquement:
				\begin{itemize}
					\item Die generierten Dateien werden wie folgt benannt:\newline
					%with a file name generated as follows:\newline
					%avec un nom de fichier généré comme suit:\newline
					[Bezeichnung]-[Kontoname].[Ausgewählte Dateiendung],
					%[name of accounting entity]-[account name].[selected extension],
					%[nom de l'entité comptable]-[nom du compte].[extension sélectionnée].
					\item und speichert sie in dem Ordner, in dem die Grisbi-Datei geöffnet wurde; Wenn die exportierte(n) Datei(en) bereits vorhanden ist/sind, werden Sie von Grisbi gefragt, ob Sie sie überschreiben möchten; Wenn Sie mit der Maus über das Menü fahren, öffnet sich das folgende Informationsfenster:\newline
					%and saves it to the folder where the Grisbi file was opened; If the exported file(s) already exist(e), Gri\end{itemize}sbi will ask you to confirm whether you want to overwrite them; hovering over the menu with the mouse opens a following information window:\newline
					%et dont la destination est le dossier où a été ouvert le fichier Grisbi; si le ou les fichiers exportés existent déjà, Grisbi vous demandera confirmation pour le ou les écraser; Le survol du menu avec la souris ouvre une fenêtre d'information suivante:\newline
					\dequote{Die generierten Dateien werden wie folgt benannt:\newline
					[Bezeichnung]-[Kontoname].[Dateiendung]\newline
					Der Speicherordner ist der Grisbi-Dateiordner}
					%\enquote{The generated files will be named as follows:\newline
					%[accounting entity]-[account name].[extension]\newline
					%The save folder will be the Grisbi file folder}
					%\frquote{Les fichiers générés seront nommés comme suit:\newline
					%[entité comptable]-[nom du compte].[extension]\newline
					%Le dossier d'enregistrement sera le dossier du fichier Grisbi}.
				\end{itemize}
				\Note{}: Es wird empfohlen, das Zeichen \dequote{\slash{}} im Kontonamen nicht zu verwenden, da dies beim Export in \gls{Linux-Distribution} einen Fehler verursachen kann.
				%\Note: it is recommended not to use the character \enquote{\slash{}} in the account name, otherwise an error may occur when exporting to \gls{Linux distributions}.
			...
			\item mit der Schaltfläche \menus{Nachfolgendes Element} button;
			%confirm with the \menus{Following} button;
			%validez par le bouton \menus{Suivant};
			\end{itemize}
		\item Geben Sie für jedes Konto den Dateinamen, das Zielverzeichnis und das Exportformat an; Mit der Option \menus{Ausgewählte Konten automatisch bearbeiten} wird dieser Schritt umgangen. Bestätigen Sie dann mit der Button \menus{Nachfolgendes Element};
		%For each account, define the name of the file, the destination directory and the export format; the option \menus{Name files automatically} bypasses this step. Then confirm with the \menus{Following} button;
	...
	\item Erscheint das Fenster zum Abschluss des Exports; Bestätigen Sie mit der Button \menus{Schließen}.
	%the export completion window appears; confirm with the \menus{Close} button;
	%la fenêtre de fin de l'exportation s'affiche; validez par le bouton \menus{Fermer}.
\end{enumerate}

\Attention{}: Im Allgemeinen ist es nicht ratsam, Akzente, Leerzeichen oder Sonderzeichen in den Namen der von Grisbi verwendeten Verzeichnisse und Dateien zu haben. Wenn ja, benennen Sie sie jetzt um. Beispielsweise können Leerzeichen durch Unterstriche (\_) ersetzt werden.
%in general, it is inadvisable to have accents, spaces or special characters in the names of directories and files used by Grisbi. If so, rename them now. For example, spaces can be replaced by underscores (\_).
