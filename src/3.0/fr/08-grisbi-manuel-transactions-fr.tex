...
\begin{itemize}
	 ...
	 \item \menus{Imprimer}: ouvre la fenêtre de sélection de l'imprimante et de ses
	options;
	 \item \menus{Affichage}: ouvre un menu déroulant; vous pouvez choisir l'\indexword{affichage des opérations}\index{affichage!opérations} parmi quatre possibilités (\menus{Vue simple}, \menus{Mode} deux lignes \frquote{}, \menus{Mode} trois lignes \frquote{} et \menus{Vue complète}), ainsi que l'affichage ou non des \indexword{opérations rapprochées}\index{affichage!opérations rapprochées} et des \indexword{lignes d'archives}\index{affichage!lignes d'archives}.
\end{itemize}

% espace pour changement de thème
\vspacepdf{5mm}
Les options du menu \menus{Affichage} influent par défaut sur l'affichage de tous les comptes. Vous pouvez préférer un affichage différent pour chaque compte, cela est configurable dans le menu \menus{Édition - Préférences} (voir le paragraphe \vref{setup-operations-list-differenciation}, \menus{Différenciation des comptes}).
...
La liste des opérations s'affiche dans le panneau des détails\refimage{transactions-list-img}.
...
Vous pouvez choisir d'afficher ou non les \indexword{opérations rapprochées}\index{affichage!opérations rapprochées} et les \indexword{lignes d'archives}\index{affichage!lignes d'archives}: sélectionnez \menus{Affichage - Montrer les opérations rapprochées} ou \menus{Affichage - Montrer les lignes d'archives} dans la barre de menus ou dans la barre d'outils (voir la section \vref{home-menus-display}, \menus{Menu Affichage} ou la fonction \menus{Affichage} dans la section \vref{transactions-functions}, \menus{Barre d'outils}).
...
Chaque opération peut être affichée sur 1, 2, 3 ou 4 lignes, suivant le mode d'affichage sélectionné (voir la section \vref{home-menus-display}, menu \menus{Affichage} dans la barre de menus ou la fonction \menus{Affichage} dans la section \vref{transactions-functions}, \menus{Barre d'outils}). L'affichage courant y est indiqué par une coche. 

Vous pouvez aussi configurer le contenu de ces lignes, ainsi que la mémorisation des réglages de l'affichage, individuellement pour chaque compte, dans le menu \menus{Édition - Préférences} (voir les sections \vref{setup-operations-list}, \menus{Comportement de la liste} et \vref{setup-operations-cells}, \menus{Cellules de la liste des opérations}).
...
Pour des raisons de lisibilité de l'affichage, Grisbi n'affiche aucune bordure aux cellules, colonnes, lignes et opérations, mais présente une alternance de couleurs de fond violet et blanc{couleur} à chaque ligne.
...
\begin{itemize}
	...
	\item \menus{Crédit};
	\item \menus{Solde}: montant toujours calculé \emph{après} l'opération affichée sur la même ligne; il dépend chronologiquement du \indexword{tri primaire}\index{tri!primaire} sélectionné; un \indexword{solde négatif}\index{solde!négatif} est affiché en rouge{couleur};
	...
	\item \menus{P/R}: statut de l'opération, \emph{Pointé} ou \emph{Rapproché};
	...
	\item \menus{Numéro d'opération}: il s'agit d'un numéro d'ordre unique que Grisbi attribue à chaque opération; vous pouvez reporter ce numéro sur la pièce comptable correspondante (\menus{Numéro d'opération} et \menus{Pièce comptable} sont des références réciproques qui assurent la traçabilité de l'opération);
	\item \menus{\No chèque} ou \menus{\No chèque/virement}, si le mode de règlement sélectionné permet cette saisie;
	\item \menus{Devise de l'opération};
	\item \menus{Taux de change};
	\item \menus{Mode de règlement de la \indexword{contre-opération}}\index{contre-opération}: vous pouvez le configurer pour chaque compte dans le menu \menus{Édition - Préférences} (voir la section \vref{setup-resources-modes}, \menus{Modes de règlement});
	\item \menus{Auto/Manuel}: statut de l'exécution d'une opération planifiée
\end{itemize}

% espace pour changement de thème
\vspacepdf{5mm}
La liste des opérations et le formulaire de saisie utilisent chacun un sous-ensemble de ces champs. Vous pouvez voir les deux listes complètes dans les tableaux du menu \menus{Édition - Préférences}, onglets \menus{Cellules de la liste des opérations} et \menus{Contenu}, ou dans les sections \vref{setup-operations-cells}, \menus{Cellules de la liste des opérations} et \vref{setup-form-content}, \menus{Contenu}.
...
Vous pouvez soit faire ces manipulations dans le menu \menus{Édition - Préférences - Opérations - Cellules de la liste des opérations}, qui offre un aperçu complet de la disposition des champs dans les cellules, et permet de faire plusieurs modifications facilement (voir la section \vref{setup-operations-cells}, \menus{Cellules de la liste des opérations}), soit le faire directement dans la liste des opérations, en suivant les méthodes ci-dessous:
...
\begin{enumerate}
	 \item sélectionnez \menus{Affichage - Vue complète} dans la barre d'outils ou \menus{Affichage - Montrer quatre lignes par opération} dans la barre de menus, pour vous assurer de voir toutes les vingt-huit cellules;
...
Pour \indexword{déplacer un champ d'information}\index{champ d'information!déplacer}, vous pouvez le supprimer puis l'ajouter dans un autre champ vide, mais vous pouvez aussi le faire dans le menu \menus{Édition - Préférences - Opérations - Cellules de la liste des opérations}. Voir la section \vref{setup-operations-cells}, \menus{Cellules de la liste des opérations}.
...
%espace pour changement de thème
\vspacepdf{5mm}
Vous pouvez aussi configurer l'ordre général des tris dans le menu \menus{Édition - Préférences} (voir la section \vref{setup-operations-list-modes}, \menus{Modes d'affichage}).
...
Le formulaire de saisie comprend par défaut les sept champs suivants: \menus{Date de l'opération}, \menus{Tiers}, \menus{Débit}, \menus{Crédit}, \menus{Catégorie}, \menus{Moyen de paiement} et \menus{Remarques}. Vous pouvez avoir besoin de champs supplémentaires, par exemple \menus{Exercice}, \menus{Imputation budgétaire} ou autre. Pour cela, allez dans le menu \menus{Édition - Préférences - Formulaire des opérations - Contenu}, ou cliquez-droit dans le formulaire de saisie, dans une zone grise en dehors des champs de saisie, sélectionnez \menus{Configurer le formulaire} (voir la section \vref{setup-form}, \menus{Formulaire des opérations}).
...
De plus, un autre menu contextuel, accessible par un clic-droit en dehors d'un champ de saisie, permet d'accéder à la \indexword{configuration du formulaire}\index{formulaire de saisie!configuration}: vous pouvez en configurer précisément le contenu, la présentation, le comportement, ainsi que les paramètres d'aide à la saisie, dans le menu \menus{Édition - Préférences} (voir la section \vref{setup-form}, \menus{Formulaire des opérations}).
...
\begin{itemize}
	 ...
	 \item dans la barre de menus, sélectionnez \menus{Édition - Nouvelle opération};
	 \item dans la barre de menus, sélectionnez \menus{Montrer le formulaire de saisie d'opérations};
...
La touche \key{Entrée} permet soit de se déplacer dans le formulaire de saisie, soit de valider la saisie; cela est configurable dans le menu \menus{Édition - Préférences} (voir la section \vref{setup-form-behaviour}, \menus{Comportement}). Cette configuration affecte à la fois la touche \key{Entrée} du clavier alphabétique et celle du pavé numérique.
...
Un double-clic sur la date, ou la combinaison de touches \key{Ctrl+Entrée}, affiche un calendrier permettant de sélectionner la date.
...
\begin{itemize}
	...
	\item un clic sur le bouton \menus{Annuler} ou l'appui sur la touche \key{Échap} ferme le calendrier sans modifier la date;
...
Vous pouvez ajuster finement ce comportement dans le menu \menus{Édition - Préférences} (voir la section \vref{setup-form-input}, \menus{Aide à la saisie du formulaire}).
...
\begin{enumerate}
	 \item créez la devise, si elle ne l'est pas déjà, dans le menu \menus{Édition - Préférences} (voir la section \vref{setup-resources-currencies}, \menus{Devises});
...
\begin{itemize}
	 ...
	 \item dans la barre de menus, sélectionnez \menus{Édition - Éditer l'opération};
...
\begin{itemize}
	 ...
	 \item dans la barre de menus, sélectionnez \menus{Édition - Supprimer une opération};
...
\begin{itemize}
	 \item dans la barre de menus, sélectionnez \menus{Édition - Utiliser l'opération sélectionnée comme modèle};
	 \item dans le menu contextuel, sélectionnez \menus{Édition - Utiliser l'opération sélectionnée comme modèle}.
\end{itemize}
...
\begin{itemize}
	 \item dans la barre de menus, sélectionnez \menus{Édition - Cloner l'opération};
...
\begin{itemize}
	 \item dans la barre de menus, sélectionnez \menus{Édition - Convertir en opération planifiée};
...
\begin{itemize}
	 \item dans la barre de menus, sélectionnez \menus{Édition - Déplacer l'opération vers un autre compte} et choisissez le compte destinataire dans la liste;
