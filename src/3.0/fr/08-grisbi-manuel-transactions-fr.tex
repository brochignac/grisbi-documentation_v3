...
\begin{figure}[htbp]
	...
	\end{center}
	\caption{Formulaire de saisie des opérations personnalisé}
...
\textbf{Note}: pour que le champ \menus{Montant} s'affiche, vous devrez configurer la liste des opérations et ajuster le nombre de lignes affichées dans la liste des opérations (voir la section \vref{setup-operations-cells}, \menus{Cellules de la liste des opérations} ou la section \vref{transactions-list-fields-manage}, \menus{Gestion des champs d'information}).

\textbf{Note}: si vous affichez une opération existante dans le formulaire de saisie, vous devrait configurer le contenu du formulaire de saisie en y ajoutant le champ \menus{Devise} pour visualiser la devise associée à l'opération en débit ou en crédit (voir la section \vref{setup-form-content}, \menus{Contenu}).
...
Le \indexword{rappel automatique}\index{opération!rappel automatique} d'une opération, si vous avez activé l'option de \menus{Remplissage automatique des opérations} dans les préférences, vous permet de saisir beaucoup plus rapidement une opération affectée à un tiers déjà connu par votre fichier Grisbi: lors de la saisie du tiers, dès que vous sortez de la zone de saisie, les autres champs du formulaire sont automatiquement complétés à l'identique de la dernière opération saisie pour ce tiers. À vous de les modifier si nécessaire avant de valider l'opération. 

Vous pouvez ajuster finement ce comportement dans le menu \menus{Éditer - Préférences} (voir la section \vref{setup-form-input}, \menus{Aide à la saisie du formulaire}).
...
% espace pour changement de thème
\vspacepdf{3mm}
Lors de la saisie de l'\indexword{opération en devise}\index{opération!devise}\index{devise!opération} dans le formulaire de saisie, choisissez dans la liste déroulante la devise dans laquelle l'opération doit être enregistrée. Plusieurs questions peuvent se poser\refimage{transactions-changeRate-img}:
...
% espace pour changement de thème
\vspacepdf{5mm}
Pour créer un tiers virtuel, créez un état contenant une liste de tiers: voir le paragraphe \vref{reportscreation-display-general-virtualThird}, \menus{Tiers virtuel}.
...
\begin{itemize}
	\item décochez la case \menus{Considérer les tiers de ce rapport comme un tiers virtuel} dans l'état qui le définit, puis validez (voir la section \vref{reports-modify}, \menus{Modification d'un état});
...
%	\includegraphics[width=0.95\textwidth]{image/screenshot/transactions_breakdown}
	...
	% image centrée 
	 ...
	 \item dans le formulaire, le curseur est dans le champ \menus{Débit}, et les champs \menus{Date}, \menus{Tiers} et \menus{Moyen de paiement} sont grisés, ce qui signifie qu'on ne peut plus les modifier: il s'agit du formulaire de sous-opération, qui est prêt pour la saisie des sous-opérations;
...
%	\includegraphics[width=0.95\textwidth]{image/screenshot/transactions_breakdown_sub1}
	...
	% image centrée 
	 ...
	 \item il y a au moins deux sous-opérations dans une opération ventilée (sinon cela n'a aucun intérêt\ldots); donc, dans le formulaire de sous-opération, saisissez une deuxième sous-opération de la même manière, puis validez: cette sous-opération s'affiche sur fond rose{couleur} et une nouvelle ligne apparaît, sur fond rouge{couleur} donc sélectionnée, qui affiche le dernier total des sous-opérations saisies et le dernier écart \refimage{transactions-breakdown-sub2-img};
...
Cependant, pour une opération ventilée, un champ supplémentaire \menus{Restaurer les sous-opérations} apparaît en bas du formulaire de saisie, à gauche des boutons \menus{Annuler} et \menus{Valider}, et vous pouvez cocher la case correspondante en cliquant dessus ou sur le champ, et selon votre besoin.
...
\begin{itemize}
	 ...
	 \item dans le menu contextuel, sélectionnez \menus{Éditer l'opération};
...
\begin{itemize}
	 ...
	 \item dans le menu contextuel, sélectionnez \menus{Supprimer l'opération}.
\end{itemize}
