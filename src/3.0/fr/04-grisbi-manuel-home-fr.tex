...
\begin{itemize}%[\large \textcircled{\small 2}]
	\item[\large\textcircled{\small 1}] la barre de titre, qui affiche un des noms suivants (disponible dans les préférences);
		\begin{itemize}
...
\textbf{Note}: la barre d'information peut être cachée par une case à décocher dans les préférences. Elle est affichée par défaut.
...
\begin{wrapfigure}{l}{0.3\textwidth}%50mm}
%\begin{figure}			% h=here, t=top, b=bottom, p=page of float to force the figure here, not in a next page.
%	\begin{center}
	\vspace{-\intextsep}	% removes the white box above the floating
	\includegraphics[width=0.3\textwidth]{image/screenshot/home_navigation}
	%	\vspace{-\intextsep}%textfloatsep}
\vspace{-45pt}			% removes the white box on the following page
%	\end{center}
%\caption{Page d'accueil}		% sous-titre/subtitle
...
\textbf{Note}: ces triangles seront remplacés, en fonction du thème de l'environnement de bureau ou du gestionnaire de fenêtres que vous utilisez, par d'autres caractères tels que +, -, >, <, etc.
