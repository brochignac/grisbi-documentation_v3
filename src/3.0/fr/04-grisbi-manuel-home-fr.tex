...
En sélectionnant l'onglet \menu{Comptes}, le panneau des détails affiche:

\begin{itemize}
	\item en haut, dans un bandeau gris foncé:
		\begin{itemize}
	 		\item à gauche, l'icône de \menu{Grisbi} (qui peut être masquée, voir \vref{setup-display-logo-icon}),
	 		\item et à droite un \indexword{titre}\index{affichage!titre}\index{titre!page d'accueil} qui permet d'identifier sur quelle comptabilité vous travaillez actuellement, sous la forme \frquote{libellé - Grisbi}; vous pouvez définir ce libellé, parmi trois possibilités, dans le menu \menu{Édition - Préférences} (voir le paragraphe \vref{setup-display-addresses-titles}, Adresses et titres):
				\begin{itemize}
	...
	\item dans la partie principale gris clair sous le bandeau:
		\begin{itemize} 
			...
			\newline
			\textbf{Note}: vous pouvez ajuster l'ordre d'affichage des soldes partiels des groupes de comptes (voir le paragraphe \vref{setup-general-home-partBalance}, \menu{Position dans la liste des soldes partiels}).			 
...
Un solde supérieur au \menu{Solde minimal voulu} s'affiche en \textcolor[RGB]{0,126,0}{vert}; au passage du pointeur sur sa ligne, cette couleur passe au \textcolor[RGB]{0,227,0}{vert clair}.

Un solde inférieur au \menu{Solde minimal voulu} et supérieur au \menu{Solde minimal autorisé} s'affiche en \textcolor[RGB]{230,155,0}{orange}; au passage du pointeur sur sa ligne, cette couleur passe à l'\textcolor[RGB]{255,200,0}{orange clair}.

Un solde inférieur au \menu{Solde minimal autorisé} s'affiche en \textcolor[RGB]{153,0,0}{rouge foncé}; au passage du pointeur sur sa ligne, cette couleur passe au \textcolor{red}{rouge}.
...
Un solde partiel, qui correspond à un groupe de comptes, s'affiche en \textcolor[RGB]{40,40,255}{bleu foncé} (comme dans la figure \vref{home_details}). S'il est négatif, il peut s'afficher en \textcolor[RGB]{153,0,0}{rouge foncé}, mais seulement si cela a été configuré ainsi (voir le paragraphe \vref{setup-general-home-partBalance}, \menu{Soldes partiels de la liste des comptes}). Une ligne de solde partiel ne change pas de couleur au passage du pointeur de la souris dessus, car on ne peut pas afficher les opérations d'un groupe de comptes.

% espace pour changement de thème
\vspacepdf{3mm}
Vous pouvez configurer certains aspects de l'affichage du panneau des détails:
\begin{itemize}
	\item dans le menu \menu{Édition - Préférences}:%TODO verify sections/paragraphs with \vref{setup-xxx}
		\begin{itemize}
			\item \menu{\indexword{Géneralités}\index{généralités}}:
				\begin{itemize}
					\item \menu{Paramètres divers}, onglet \menu{Échéancier}\index{échéancier}: section \vref{setup-general-planned};
					\item \menu{Acceuil}:
						\begin{itemize}
							\item \menu{Calcul des soldes}: paragraphe \vref{setup-general-home-balance},
							\item \menu{Position dans la liste des soldes partiels}: paragraphe \vref{setup-general-home-partBalance};
						\end{itemize}
				\end{itemize}
			\item \menu{\indexword{Affichage}\index{affichage}}:
				\begin{itemize}
					\item \menu{Polices et Logo}\index{polices}\index{logo}: section \vref{setup-display-logo},
					\item \menu{Adresses et titres}\index{titre}\index{adresses}: section \vref{setup-display-addresses-titles};
				\end{itemize}
		\end{itemize}
	\item dans l'onglet \menu{Propriétés}\index{propriétés} de chaque compte à la rubrique \menu{Soldes}:
		\begin{itemize}
			 \item comptes sous le \menu{Solde minimal autorisé}\index{solde!minimal autorisé}: section \vref{accounts-properties},
			 \item comptes sous le \menu{Solde minimal voulu}\index{solde!minimal voulu}: section \vref{accounts-properties}.
		\end{itemize}
\end{itemize}
...
\begin{itemize}[rightmargin=.6cm]
	\item \menu{Nouvelle fenêtre}: non fonctionnel (dans le futur peut-être?);	%TODO: to update
\end{itemize}

\noindent
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menu{Nouveau fichier de comptes}: crée un nouveau fichier Grisbi d'\gls{extension} \file{.gsb}; le fichier courant est donc fermé et un nouveau fichier vide est créé avec un compte vide (raccourci-clavier \key{Ctrl}\key{N}), voir la section \vref{start-newfile}; à ne pas confondre avec la création d'un nouveau compte;	 
		\item \menu{Ouvrir}: ouvre votre gestionnaire de fichiers, permettant de rechercher, sélectionner et ouvrir un fichier de comptes d'\gls{extension} \file{.gsb} (raccourci-clavier \key{Ctrl}\key{O});
...
\begin{minipage}{.3\linewidth}
	...
	\captionsetup{
		...
		name=Fig.,%					% rename label caption, default="Figure" (or "Table")
		labelsep=newline}			% define the separator between the label and the caption text
	\caption{Menu \menu{Fichier}}	% \caption is mandatory to reference a figure in lof
...
\begin{itemize}
	...
	\item \menu{Enregistrer sous}: ouvre un gestionnaire de fichiers pour enregistrer le fichier de comptes en cours avec le nom et à l'emplacement de votre choix; Grisbi vous propose par défaut le répertoire courant, le nom du fichier de comptes en cours, avec l'\gls{extension} \file{.gsb};
	\item \menu{Importer un fichier}: démarre l'assistant d'importation de fichiers d'un autre logiciel (raccourci-clavier \key{Ctrl}\key{I}); voir la section \vref{move-import-importinit};
	\item \menu{Exporter vers un fichier QIF/CSV}: démarre l'assistant d'exportation de fichiers de compte (raccourci-clavier \key{Ctrl}\key{E}); voir la section \vref{move-export};	
	...
	\item \menu{Déboguer le fichier de comptes}: démarre l'assistant de débogage de ce fichier, qui va vous aider à chercher des incohérences dans votre fichier de comptes; voir la section \vref{maintenance-file-debug};
	\item \menu{Rendre anonyme le fichier de comptes}: démarre l'assistant qui produit une copie anonymée (de manière irréversible) de votre fichier de comptes; ce fichier pourra être joint à un rapport de bogue; voir la section \vref{maintenance-file-anonymous};	
	\item \menu{Rendre anonyme le fichier QIF}: démarre l'assistant qui produit une copie anonymée (de manière irréversible) de ce fichier; ce fichier pourra être joint à un rapport de bogue; voir la section \vref{maintenance-QIF-anonymous};	
...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menu{Éditer l'opération}: permet la rectification d'une opération existante, voir la section \vref{transactions-modify}, \menu{Modification d'une opération};
		\item \menu{Nouvelle opération}: permet la création d'une nouvelle opération dans un compte (raccourci-clavier \key{Ctrl}\key{T}), voir la section \vref{transactions-new}, \menu{Saisie d'une nouvelle opération};
		\item \menu{Supprimer une opération}: supprime une opération existante, voir la section \vref{transactions-delete}, \menu{Suppression d'une opération};
	\end{itemize}
\end{minipage}
\hspace{10pt}	
\begin{minipage}{.3\linewidth}
	...
	\captionsetup{
		...
		name=Fig.,%					% rename label caption, default="Figure" (or "Table")
		labelsep=newline}			% define the separator between the label and the caption text
	\caption{Menu \menu{Éditer}}	% \caption is mandatory to reference a figure in lof
	%\vspace{30pt}					% space below the caption
...
\item \menu{Cloner l'opération}: permet de dupliquer une opération existante, voir la section \vref{transactions-duplicate}, \menu{Clonage d'une opération};
...
\item \menu{Rechercher}: affiche la fenêtre de	création d'un état de recherche, voir le chapitre \vref{reportscreation}, \menu{Création d'un état};
\item \menu{Nouveau compte}: démarre l'assistant de	création d'un nouveau compte dans votre fichier Grisbi (raccourci-clavier \key{Maj}\key{Ctrl}\key{N}), voir la section \vref{accounts-new}, \menu{Création d'un nouveau compte};
\item \menu{Supprimer le compte courant}: efface le compte sélectionné de votre fichier Grisbi, voir la section \vref{accounts-delete}, \menu{Suppression d'un compte};
\item \menu{Préférences}: permet de configurer Grisbi (raccourci-clavier \key{Maj}\key{Ctrl}\key{P}); voir le chapitre \vref{setup}, \menu{Configuration de Grisbi}.
\end{itemize}
...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menu{Montrer le formulaire de saisie d'opérations}: permet de développer le formulaire de saisie des opérations du compte sélectionné;
		\item \menu{Montrer les opérations rapprochées}: permet l'affichage des opérations rapprochées du compte sélectionné (raccourci-clavier \key{Alt}\key{R});
		\item \menu{Montrer les lignes d'archives}: affiche les lignes d'archives du compte sélectionné (raccourci-clavier \key{Altl}\key{L});
	\end{itemize}
\end{minipage}
\hspace{10pt}	
\begin{minipage}{.3\linewidth}
	...
	\captionsetup{
		...
		name=Fig.,%					% rename label caption, default="Figure" (or "Table")
		labelsep=newline}			% define the separator between the label and the caption text
	\caption{Menu \menu{Affichage}}	% \caption is mandatory to reference a figure in lof
	%\vspace{30pt}					% space below the caption
	\label{home_menubar_view}
\end{minipage} 
\begin{itemize}
	\item \menu{Montrer les \indexword{comptes clos}}\index{compte!clos}: affiche le(s) compte(s) clos et non supprimé(s), voir la section \vref{accounts-properties}, \menu{Propriétés d'un compte};
...
\end{addmargin*}	
	\item \menu{Réinitialiser la largeur des colonnes}: permet de remettre les colonnes des listes d'opérations du compte sélectionné ou de l'échéancier à leur largeur d'origine.
\end{itemize}
...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menu{Manuel}: ouvre votre navigateur à la page \frquote{Manuel de l'Utilisateur de Grisbi} (raccourci-clavier \key{F1});
		\item \menu{Démarrage rapide}: ouvre votre navigateur à la page \frquote{Démarrage Rapide de Grisbi};
		\item \menu{À propos}: affiche la fenêtre d'information sur l'application; vous y trouverez des détails sur la version, le lien vers le site de Grisbi, les remerciements (contributeurs au projet) et la licence d'utilisation;
	\end{itemize}
\end{minipage}
\hspace{10pt}	
\begin{minipage}{.3\linewidth}
	...
	\captionsetup{
		...
		name=Fig.,%					% rename label caption, default="Figure" (or "Table")
		labelsep=newline}			% define the separator between the label and the caption text
	\caption{Menu \menu{Aide}}	% \caption is mandatory to reference a figure in lof
	%\vspace{30pt}					% space below the caption
...
Grisbi dispose d'un certain nombre de raccourcis-clavier, listés dans les préférences de Grisbi (voir \vref{{setup-display-toolbars}}), présentés ici selon différents thèmes, (voir aussi la section \vref{introduction-manual-conventions}, \menu{Conventions typographiques du présent manuel}).


\subsection{Application et fichiers}

\begin{itemize}
	\item créer un Nouveau fichier Grisbi: \key{Ctrl}\key{N};
	\item Ouvrir un fichier Grisbi: \key{Ctrl}\key{O};
	\item ajouter un Nouveau compte au fichier Grisbi: \key{Ctrl}\key{Maj}\key{N};
	\item Enregistrer le fichier Grisbi: \key{Ctrl}\key{S};
	\item Importer un fichier: \key{Ctrl}\key{I};
	\item Exporter vers un fichier \gls{QIF}/\gls{CSV}: \key{Ctrl}\key{E};
	\item Fermer le fichier Grisbi: \key{Ctrl}\key{W};
...
\begin{itemize}
	\item Sélectionner un onglet ou un compte: \key{Flèche Haut}, \key{Flèche Bas};
	\item Sur l'onglet Comptes:
		\begin{itemize}
			\item Ouvrir la liste des comptes: \key{Flèche Droit};
			\item Fermer la liste des comptes:  \key{Flèche Gauche};
		\end{itemize}
	\item Sur un compte ou sur l'onglet Échéancier:
		\begin{itemize}
			\item Bascule vers la liste des opérations ou des opérations planifiées: \key{Flèche Droit};
			\item Bascule vers le panneau de navigation: \key{Flèche Gauche};
		\end{itemize}
	\item Sur un compte/l'onglet Échéancier, ouvrir/fermer la liste des comptes:
	\item Sélectionner le premier/dernier onglet (Comptes/États): \key{Page Préc/\PgUpSymbol} ou \key{Page Suiv/\PgDownSymbol}.
\end{itemize}
...
\begin{itemize}
	...
	\item Nouvelle opération:
		\begin{itemize}
			\item Sur une ligne vide: \key{Entrée};
			\item Sur une opération existante: \key{Ctrl}\key{T};
		\end{itemize}
	...
	\item Rapprocher ou dé-rapprocher une opération: \key{Ctrl}\key{R};
	\item Montrer ou masquer les opérations rapprochées: \key{Alt}\key{R};
	\item Montrer ou masquer les lignes d'archives: \key{Alt}\key{L}.
\end{itemize}
...
\begin{itemize}
	\item La touche \key{Entrée} est configurable (voir les préférences \vref{setup-form-behaviour-enter}): elle permet soit de se déplacer dans le formulaire de saisie, soit de valider l'entrée;
	...
	\item Symbole de l'euro: \key{Alt Gr}\key{E}.
\end{itemize}
...
\begin{itemize}
	\item Ouvrir une liste: \key{Page Préc/\PgUpSymbol}, \key{Page Suiv/\PgDownSymbol} ou \key{Flèche bas};
	\item Se placer en haut de la liste: \key{Page Préc/\PgUpSymbol};
	\item Se placer en bas de la liste: \key{Page Suiv/\PgDownSymbol};
	\item Se déplacer dans la liste: \key{Flèche haut} ou \key{Flèche bas};
	\item Valider un choix à l'intérieur d'une liste: \key{Entrée} ou \key{Tabulation} en fonction de votre configuration (ici \vref{setup-form-behaviour-enter});
...
\begin{itemize}
	\item Ouvrir un calendrier mensuel (sur le champ de date): \key{Ctrl}\key{Entrée};
	\item Fermer le calendrier sans modifier la date: \key{Échap};
	\item Valider la date sélectionnée: \key{Entrée};
	...
	\item Mois précédent ou suivant: \key{Page Préc/\PgUpSymbol} ou \key{Page Suiv/\PgDownSymbol};
	\item Premier jour ou dernier jour du mois: \key{Début/Orig/\,$\nwarrow$\,} ou \key{Fin}.
\end{itemize}
...
\begin{itemize}
	...
	\item Semaine précédente ou suivante: \key{Ctrl} \key{+} ou \key{Ctrl} \key{-};
	\item Mois précédent ou suivant: \key{Page Suiv/\PgDownSymbol} ou \key{Page Préc/\PgUpSymbol};
	\item Année précédente ou suivante: \key{Ctrl} \key{Page Suiv/\PgDownSymbol} ou \key{Ctrl} \key{Page Préc/\PgUpSymbol};
...
\begin{itemize}
	\item Déplacer la sélection: \key{Flèche Haut}, \key{Flèche Bas}, \key{Page Préc/\PgUpSymbol} ou \key{Page Suiv/\PgDownSymbol};
	\item Afficher les sous-catégories ou sous-imputations budgétaires (sur une catégorie ou une imputation budgétaire): \key{+};
	\item Afficher les opérations des sous-catégories ou sous-imputations budgétaires (sur une sous-catégorie ou une sous-imputation budgétaire): \key{Entée}.
\end{itemize}
...
\begin{itemize}
	\item Afficher la fenêtre des préférences de Grisbi: \key{Ctrl}\key{Maj}\key{P}.
\end{itemize}

\subsection{Aide}

\begin{itemize}
	\item Ouvrir le Manuel de Grisbi: \key{F1}.
\end{itemize}
