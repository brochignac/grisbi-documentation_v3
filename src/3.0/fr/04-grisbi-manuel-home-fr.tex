...
\begin{itemize}
	...
	\item \menus{Exporter vers un fichier QIF/CSV}: démarre l'assistant d'exportation de fichiers de compte (raccourci-clavier \keys{Ctrl+E}); voir la section \vref{move-export};	
	\item \menus{Créer une archive}: démarre l'assistant de création d'archive; voir la section \vref{datamanagement-history-new};	
	\item \menus{Exporter une archive vers un fichier GSB/QIF/CSV}: démarre l'assistant d'exportation d'archive; voir la section \vref{datamanagement-history-export};
	...
	\item \menus{Rendre anonyme le fichier QIF}: démarre l'assistant qui produit une copie anonymée (de manière irréversible) de ce fichier; ce fichier pourra être joint à un rapport de bogue; voir la section \vref{maintenance-QIF-anonymous};	
...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menus{Éditer l'opération}: permet la rectification d'une opération existante, voir la section \vref{transactions-modify}, \menus{Modification d'une opération};
		...
		\item \menus{Supprimer une opération}: supprime une opération existante, voir la section \vref{transactions-delete}, \menus{Suppression d'une opération};
	\end{itemize}
\end{minipage}
\hspace{10pt}	
\begin{minipage}{.3\linewidth}
...
\begin{itemize}	
\item \menus{Utiliser l'opération sélectionnée comme modèle}: voir la section \vref{transactions-model}, \menus{Opération sélectionnée comme modèle};
...
\item \menus{Rechercher}: affiche la fenêtre de	création d'un état de recherche, voir le chapitre \vref{reportscreation}, \menus{Création d'un état};
...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menus{Montrer le formulaire de saisie d'opérations}: permet de développer le formulaire de saisie des opérations du compte sélectionné;
...
\begin{minipage}{.7\linewidth}
	\begin{itemize}[rightmargin=.6cm]
		\item \menus{Manuel}: ouvre votre navigateur à la page \frquote{Manuel de l'Utilisateur de Grisbi} (raccourci-clavier \keys{F1});
...
\begin{itemize}
	...
	\item Sélectionner le premier/dernier onglet (Comptes/États): \keys{Pg.Préc / \PgUpSymbol} ou \keys{Pg.Suiv / \PgDownSymbol}.
\end{itemize}
...
\begin{itemize}
	\item Ouvrir une liste: \keys{Pg.Préc / \PgUpSymbol}, \keys{Pg.Suiv / \PgDownSymbol} ou \keys{\arrowkeydown};
	\item Se placer en haut de la liste: \keys{Pg.Préc / \PgUpSymbol};
	\item Se placer en bas de la liste: \keys{Pg.Suiv / \PgDownSymbol};
...
\begin{itemize}
	...
	\item Mois précédent ou suivant: \keys{Pg.Préc / \PgUpSymbol} ou \keys{Pg.Suiv / \PgDownSymbol};
...
\begin{itemize}
	...
	\item Mois précédent ou suivant: \keys{Pg.Suiv / \PgDownSymbol} ou \keys{Pg.Préc / \PgUpSymbol};
	\item Année précédente ou suivante: \keys{Ctrl+Pg.Suiv / \PgDownSymbol} ou \keys{Ctrl+Pg.Préc / \PgUpSymbol};
...
\begin{itemize}
	\item Déplacer la sélection: \keys{\arrowkeyup}, \keys{\arrowkeydown}, \keys{Pg.Préc / \PgUpSymbol} ou \keys{Pg.Suiv / \PgDownSymbol};
	\item Afficher les sous-catégories ou sous-imputations budgétaires (sur une catégorie ou une imputation budgétaire): \keys{{+}};
	\item Afficher les opérations des sous-catégories ou sous-imputations budgétaires (sur une sous-catégorie ou une sous-imputation budgétaire): \keys{Entée \return}.
\end{itemize}
...
\begin{itemize}
	\item Afficher la fenêtre des préférences de Grisbi: \keys{Ctrl}\key{Maj}\key{P}.
\end{itemize}
