...
\begin{figure}[htbp]			% h=here, t=top, b=bottom, p=page of float to force the figure here, not in a next page.
\begin{center}
\includegraphics[width=1\textwidth]{image/screenshot/home\_3.0.png}
\end{center}
...
\begin{wrapfigure}{l}{0.3\textwidth}%50mm}
	...
	\includegraphics[width=0.29\textwidth]{image/screenshot/home_navigation}
...
\begin{wrapfigure}{l}{0.3\textwidth}%50mm}
\vspace{-\intextsep}				% decreases space above the floating (minus intextsep=separation between the float and the text, in the text)
\centering							% centering the floating figure in the "wrap"
\includegraphics[width=0.29\textwidth]{image/screenshot/home_navigation}%
\vspace{-5pt}						% decreases space between the floating and the caption below the floating
\captionsetup{%						% for a change of options of the caption package
	format=plain,					% to avoid error message with labelsep option below, default=hang
	name=Fig.,						% rename label caption, default="Figure" (or "Table")
	justification=centering,		% centring of text and caption label 
...
\caption{Panneau de navigation}		% \caption is mandatory to reference a figure in lof
\vspace{-10pt}						% decreases espace below the caption
\label{home_navigation}
\end{wrapfigure}
...
\begin{itemize}
	 \item dans un pavé sur fond gris, l'icône \menu{Grisbi} à gauche, et à droite un \indexword{titre}\index{affichage!titre}\index{titre!page d'accueil} qui permet d'identifier sur quelle comptabilité vous travaillez actuellement, sous la forme \frquote{libellé - Grisbi}; vous pouvez définir ce libellé, parmi trois possibilités, dans le menu \menu{Édition - Préférences} (voir le paragraphe \vref{setup-display-addresses-titles}, \menu{Titres}):
		\begin{itemize}
			 ...
			 \item le \menu{Nom du fichier de comptes}: c'est le nom du fichier dans le répertoire courant, sous la forme \file{nom\_de\_votre\_fichier.gsb}, et c'est le choix par défaut;
		\end{itemize}
