...
\begin{itemize}%[\large \textcircled{\small 2}]
	\item[\large\textcircled{\small 1}] la barre de titre, qui affiche un des noms suivants (disponible dans les préférences \vref{setup-dislpay-addresses-titles})
		\begin{itemize}
...
\textbf{Note}: la barre d'information peut être cachée par une case à décocher dans les préférences. Elle est affichée par défaut \vref{setup-display-toolbars}
...
\begin{wrapfigure}{l}{0.3\textwidth}%50mm}
	...
	\vspace{-80pt}			% removes the white box on the following page
%	\captionformat{\newline}
%	singlelinecheck=false
	\captionsetup{name=Fig.}%,singlelinecheck=false}%,labelsep=newline}
