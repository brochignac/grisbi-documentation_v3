...
% $Id: 07-grisbi-manuel-account-fr.tex, v 3.0 2025/12 Dominique Brochard
...
\begin{itemize}
	 ...
	 \item déplacez la sélection avec les touches du clavier \keys{\arrowkeyup}, \keys{\arrowkeydown};
...
\begin{itemize}
	 \item l'onglet \menus{Opérations}, qui contient la liste des opérations (voir le chapitre \vref{transactions} Opérations d'un compte);
	 \item l'onglet \menus{Propriétés}.
\end{itemize}
...
\begin{itemize}
	 \item soit les onglets \menus{Prévisions} et \menus{Données historiques}, pour les comptes de banque ou de caisse;
	 \item soit l'onglet \menus{Tableau d'amortissement}, pour les comptes de passif.
\end{itemize}
...
\end{center}
\caption{Onglet \menus{Propriétés} d'un compte}
...
\begin{enumerate}
	\item \menus{Details du compte N°x}:
		\begin{itemize}
			\item \menus{Nom du compte}: ce peut-être un nom quelconque, par exemple \emph{Compte courant}, \emph{Compte livret}, \emph{Compte banque 1} ou \emph{Compte banque 2};	
			% saut de ligne pour indentation correcte de la note dans la liste

			\Note{}: dans le cas d'une comptabilité numérotée (association ou petite entreprise), les noms des comptes sont précédés du numéro fourni par le plan comptable, par exemple \frquote{512. Compte banque X }; ce libellé est donc une donnée alphanumérique.

			\item \menus{Type du compte}: pour plus de détails, voir la section \vref{accounts-type};	
			\item \menus{Devise du compte}: la devise dans laquelle les opérations de ce compte sont libellées (voir les sections \vref{setup-resources-currencies} et \vref{setup-resources-rate});	
			\item \menus{{Compte clos}}\index{compte!clos}: case à cocher qui vous permet de masquer le compte dans le panneau de navigation et dans la barre d'information, sans pour autant le supprimer; il pourra être affiché à nouveau en cliquant sur le menu \menus{Affichage - Montrer les comptes clos};
		\end{itemize}
	\item \menus{Titulaire du compte}:
		\begin{itemize}
			\item \menus{Nom du titulaire}: le titulaire d'un compte peut être différent pour chaque compte, par exemple si vous ouvrez un livret d'épargne pour chacun de vos enfants;	
			\item \menus{Adresse du titulaire}: le titulaire peut avoir la même adresse que le titulaire principal, ou bien une adresse différente qu'il faudra préciser ici après avoir coché la case \menus{Adresse du titulaire} (voir le paragraphe \vref{setup-display-addresses-address}, \menus{Adresses});
		\end{itemize}
	\item \menus{Banque}:
		\begin{itemize}
			\item \menus{Établissement financier}: la banque qui gère votre compte; vous pouvez choisir une banque indiquée dans la liste déroulante, ou bien cliquer sur \menus{Ajouter une nouvelle banque}; il est conseillé de renseigner ce champ (voir la section \vref{setup-resources-banks});
			\item \menus{Code BIC}:	après avoir sélectionné une banque existante, vous pourrez cliquer sur le bouton \menus{Modifier} ouvrant une fenêtre pour éditer les paramètres de la banque;
			\item \menus{N° \gls{IBAN}}: le code que vous trouverez sur votre relevé d'identité bancaire (RIB) ou IBAN fourni par la banque (voir la section \vref{setup-resources-banks});
			\item \menus{Code Banque}: affiche le code de l'établissement financier sélectionné au-dessus;	
			\item \menus{Guichet/Agence}: l'adresse de votre agence bancaire, à relever également sur votre RIB (voir la section \vref{setup-resources-banks});	
			\item \menus{Numéro du compte/Clé}: voir sur votre RIB (voir la section \vref{setup-resources-banks});
		\end{itemize}
	\item \menus{Soldes}:
		\begin{itemize}
			\item \menus{Solde initial}: le solde de votre dernier relevé bancaire au moment de la création du compte; il est conseillé de laisser le solde initial du compte à zéro et de créer par la suite une opération initiale du montant nécessaire, car cela facilitera la gestion future des rapprochements;	
			\item \menus{Solde minimal autorisé}: le solde minimal que vous autorise votre établissement financier, plus communément appelé autorisation de découvert;	
			\item \menus{Solde minimal voulu}: le solde minimal que vous aimeriez respecter; pour un bon fonctionnement de la mise en couleur des soldes dans la page \menus{Accueil}, il doit être supérieur ou égal au solde minimal autorisé;
		\end{itemize}	
	\item \menus{Commentaires}: zone de saisie pour des commentaires informels sur le compte: date d'ouverture, taux de rendement, etc. (voir la section \vref{setup-resources-banks}).
\end{enumerate}
...
\begin{itemize}
	\item soit en cliquant sur le menu \menus{Éditer - Nouveau compte} (ou avec \keys{Ctrl+Maj+N});
	\item soit en faisant un clic droit dans le panneau de navigation, sur l'onglet \menus{Comptes} ou sur un compte existant et en cliquant sur \menus{Nouveau compte} dans le menu contextuel.
\end{itemize}

L'assistant \menus{Créer un nouveau compte} comprenant quatre étapes, s'ouvre:
\begin{enumerate}
	\item accueil de l'assistant (étape 1/4); validez par le bouton \menus{Suivant};
	\item \menus{Sélection du type de compte} (étape 2/4): faites votre choix parmi les quatre types de compte proposés, puis validez par le bouton \menus{Suivant};
	...
	\item la fenêtre suivante vous permet de paramétrer ce nouveau compte \refimage{account-new-creation-img}:
		% image centrée
		\begin{figure}[htbp]
		\begin{center}
		\includegraphics[width=0.95\textwidth]{image/screenshot/account_new_currency}
		\end{center}
		\caption{Paramètres du nouveau compte}
		\label{account-new-creation-img}
		\end{figure}
		% image centrée
		\begin{enumerate}
			\item choisissez la \indexword{devise}\index{devise} du compte dans la liste déroulante, qui propose les devises que connaît déjà votre fichier Grisbi; sinon, cliquez sur \menus{Ajouter/Changer}, une fenêtre s'affiche, où une liste déroulante vous propose toutes les devises du monde connues par Grisbi; les détails de la devise sélectionnée s'affichent en-dessous, dans des champs que vous pouvez encore modifier; puis validez par le bouton \menus{Fermer},
			\item choisissez la \indexword{banque}\index{banque!création} qui détient ce compte dans la liste déroulante et qui propose les banques que connaît déjà votre fichier Grisbi; sinon, cliquez sur \menus{Ajouter une nouvelle banque}: une fenêtre s'affiche, où vous pouvez renseigner le nom de la banque et de nombreux autres détails; puis validez par le bouton \menus{Fermer},
			...
			\item saisissez le solde initial, puis validez par le bouton \menus{Suivant};
			% espace pour Note ou Attention à la ligne dans une liste
		...
		\end{enumerate}
	\item la dernière fenêtre\refimage{account-new-name-img} vous permet de saisir le nom du nouveau compte, celui qui apparaîtra dans le panneau de navigation; puis validez par le bouton \menus{Fermer};
		% image centrée
		\begin{figure}[htbp]
		\begin{center}
...
\begin{itemize}
	\item rubrique \menus{Détails du compte N°x}:
		\begin{itemize}
			\item le \menus{Nom du compte};
			\item le \menus{Type du compte};
			\item la \menus{Devise du compte};
		\end{itemize}
	\item rubrique \menus{Soldes}:
		\begin{itemize}
			\item le \menus{Solde initial}.
		\end{itemize}
\end{itemize}
...
\begin{itemize}
	 ...
	 \item \menus{Tout sélectionner} dans le champ renseigné;
	 \item \menus{Insérer un émoji}\index{émoji}: ouvre une fenêtre donnant accès à toutes sortes d'émoticônes et d'icônes.
\end{itemize}
...
\begin{itemize}
	\item soit en sélectionnant un compte, avec l'onglet \menus{Opérations} affiché dans le panneau des détails, puis en cliquant sur le menu \menus{Éditer - Supprimer le compte courant} de la barre de menu,
	\item soit par un clic droit sur un compte dans le panneau de navigation qui ouvre un menu contextuel donnant accès à l'option \menus{Supprimer ce compte}.
\end{itemize}
...
\vspacepdf{3mm}
\Attention{}: il n'y aura pas d'autre avertissement, et le compte, ainsi que toutes ses opérations, sera supprimé. Cette action est \indexword{\textbf{irréversible}}\index{action!irréversible}!
...
\begin{itemize}
	\item compte bancaire: il permet de faire tout type d'opération (voir \vref{accounts-type-bank});
	\item compte de caisse: il ne permet de gérer que des espèces (voir \vref{accounts-type-cash});
	\item compte de passif: il permet de gérer une dette que vous remboursez (voir \vref{accounts-type-liabilities}); 
	\item compte d'actif: il permet de gérer un bien qui se déprécie avec le temps (voir \vref{accounts-type-assets}).
\end{itemize}
...
\subsection{Type \emph{compte bancaire}\label{accounts-type-bank}}
...
\begin{tblr}{
	%width=0.95\textwidth,
	colspec={Q[c] Q[3.2cm,c] Q[c] Q[5.5cm,c] Q[3.2cm,c]},
	...
	\menus{Date} & \menus{Tiers} & \menus{Crédit} & \menus{Catégories} & \menus{Mode de  règlement} \\
	jj/mm/aa 	 & Dupond		 & 20,00		  & Cotisations		   & \menus{Chèque}				\\
	jj/mm/aa	 & Durant		 & 40,00		  & Cotisations		   & \menus{Chèque}				\\
\end{tblr}
...
\begin{tblr}{
	...
	\menus{Date} & \menus{Tiers}	 & \menus{Crédit} & \menus{Catégories}					 & \menus{Mode de  règlement} \\
	jj/mm/aa 	 & Remise de chèques & 60,00		  & \emph{Virement: Chèques à encaisser} & \menus{Chèque}			  \\
\end{tblr}
...
\subsection{Type \emph{compte de caisse}\label{accounts-type-cash}}
...
\subsection{Type \emph{compte de passif}\label{accounts-type-liabilities}}
...
\begin{tblr}{
	%width=0.95\textwidth,
	colspec={Q[c] Q[2.7cm,c] Q[c] Q[3.8cm,c] Q[4.8cm,c]},
	%colspec={Q[1cm,r] Q[1.5cm,c] Q[1cm,l]},
	%rowspec={Q[m] Q[m] Q[m] Q[m] Q[m]},
	%rowsep=4.5pt,
	leftsep=4pt,							%default=6
	...
	\menus{Date} & \menus{Tiers}	& \menus{Débit} & \menus{Catégories}		  & \menus{Remarques}			 \\
	jj/mm/aa 	 & Crédit Bitoire	& 20 000.00		& \emph{Virement: Banque Yze} & Dette emprunt Crédit Bitoire \\
\end{tblr}
...
Par exemple, à chaque échéance, vous saisissez une opération ventilée sur votre compte courant pour obtenir une opération celle-ci:
...
\begin{tblr}{
		%vspan=even,
		%hspan=minimal,
		hline{2-3}={2-4}{lightgray,solid},
		%hline{4}={2-3}{solid},
		%hline{4}={4-5}{1pt},
		%hline{5-7}={4,5}{solid},
		vline{2,3,4,5}={2}{lightgray,solid},
		%width=0.95\textwidth,
		colspec={Q[2cm,l] Q[2.5cm,r] Q[7cm,l] Q[1.5cm,c]},
		cell{1}{1}={c=4}{l},
		%cell{2}{2}={leftsep=2pt},
		cell{5}{1}={c=2}{l},
		cell{6}{1}={c=2}{l},
		cell{7}{1}={c=2}{l},
		cell{3,4}{2-4}={bg=blue!10!white},
		cell{5-7}{2-4}={bg=pink!80!white},
		%colspec={Q[1cm,r] Q[1.5cm,c] Q[1cm,l]},
		%rowspec={Q[m] Q[m] Q[m] Q[m] Q[m]},
		%rowsep=4.5pt,
		%leftsep=4pt,							%default=6
	...
	\textbullet{} Opération maître:	&				&					& 			   \\
									& $\blacktriangledown$ \hspace{11mm} Date			& Tiers				& Débit \\
									& jj/mm/aaaa		& Crédit Bitoire	& 350.00 	\\
									& 				& \emph{Opération ventilée} & \\
	\hspace{.3cm} \textopenbullet{} 1\up{re} opération ventilée: & & \emph{Virement: Banque Yze} &  300.00 \\
	\hspace{.3cm} \textopenbullet{} 2\up{e} opération ventilée: & & \emph{Frais financiers: Charges d'emprunts} & \SetCell{c} 48.80 \\
	\hspace{.3cm} \textopenbullet{} 3\up{e} opération ventilée: & & \emph{Assurances} & 1.20 \\
\end{tblr}
...
\subsection{Type \emph{compte d'actif}\label{accounts-type-assets}}
...
\begin{tblr}{
		%width=0.95\textwidth,
		colspec={Q[c] Q[2.7cm,c] Q[c] Q[4.6cm,c] Q[4cm,c]},
		%colspec={Q[1cm,r] Q[1.5cm,c] Q[1cm,l]},
		%rowspec={Q[m] Q[m] Q[m] Q[m] Q[m]},
		%rowsep=4.5pt,
		leftsep=4pt,							%default=6
	...
	\menus{Date} & \menus{Tiers} & \menus{Débit} & \menus{Catégories}		  			     & \menus{Remarques}   \\
	jj/mm/aa 	 & Auto Matique  & 25 000.00	 & \menus{Virement:\\} Compte Immobilisation & Actif achat voiture \\
\end{tblr}
...
\begin{tblr}{
		%width=0.95\textwidth,
		colspec={Q[c] Q[2.6cm,c] Q[c] Q[4.5cm,c] Q[4cm,c]},
		%colspec={Q[1cm,r] Q[1.5cm,c] Q[1cm,l]},
		%rowspec={Q[m] Q[m] Q[m] Q[m] Q[m]},
		%rowsep=4.5pt,
		leftsep=4pt,							%default=6
	...
	\menus{Date} & \menus{Tiers} & \menus{Débit} & \menus{Catégories}		  			     & \menus{Remarques}   \\
	31/12/2025 	 & Dépréciation Voiture  & 5 000.00	 & \menus{Virement:\\} Compte Immobilisation & Amortissements: Dépréciation \\
\end{tblr}
...
%todo modèle formulaire à améliorer
\begin{tblr}{
		width=0.95\textwidth,
		colspec={Q[.1pt,c] Q[18mm,l] Q[colsep=1pt,.1em] Q[6cm,l] Q[5mm,c] Q[colsep=1pt,.1em] Q[18mm,l] Q[colsep=1pt,.1em] Q[18mm,l] Q[.1pt]},
		cell{1-7}{1-10}={bg=lightgray!20!white},
		hlines={lightgray!20!white},
		vlines={lightgray!20!white},
		row{1,7}={rowsep=0pt,cmd=\fontsize{1}{1}},
		row{3,5}={rowsep=0pt},
		hline{2,3}={2,2}{lightgray,solid}, vline{2,3}={2,2}{gray!50!white,solid},	%date
		hline{2,3}={4,4}{lightgray,solid}, vline{4,5}={2,2}{gray!50!white,solid},	%tiers
		hline{2,3}={7,7}{lightgray,solid}, vline{7,8}={2,2}{gray!50!white,solid},	%débit
		hline{2,3}={9,9}{lightgray,solid}, vline{9,10}={2,2}{gray!50!white,solid},	%credit
		hline{4,5}={4,4}{lightgray,solid}, vline{4,5}={4,4}{gray!50!white,solid},	%catégories
		hline{4,5}={7,7}{lightgray,solid}, vline{7,8}={4,4}{gray!50!white,solid},	%carte
		hline{6,7}={4,5}{lightgray,solid}, vline{4,6}={6,6}{gray!50!white,solid},	%remarques
		vline{5}={6}{white},
		cell{2,2}{2,4,7,9}={cmd=\textsf,bg=white},
		cell{4,4}{4,7}={cmd=\textsf,bg=white},
		cell{6,6}{4,5}={cmd=\textsf,bg=white},
		abovesep=5pt,							%default=2
		stretch=0							
	}
	&		& & 								& 						& &		  & &		 & \\
	& Date	& & Tiers							& $\blacktriangledown$ 	& & Débit & & Crédit & \\
	&		& &									&						& &		  & &		 & \\
	& 		& & Catégories : Sous-catégories	& $\blacktriangledown$	& & Carte & &		 & \\
	& 		& & 								& 						& & 	  & &		 & \\
	& 		& & Remarques 						& 						& & 	  & &		 & \\
	& 		& & 								& 						& & 	  & &		 & \\
\end{tblr}
...
%Cette section devient de plus en plus obsolète au fur et à mesure que le temps passe, mais nous la maintenons encore pour vous permettre l'éventuel import de vieux fichiers Grisbi.
