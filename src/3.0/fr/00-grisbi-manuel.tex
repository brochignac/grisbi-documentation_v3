...
\hypersetup{%							% to create metadata to insert in pdf
	...
	plainpages=false					% do page number anchors as plain Arabic, default=false
%	pdfpagelabels						% set PDF page labels - Commenté car élimine  "Package hyperref Warning: Option `pdfpagelabels' has already been used,"
}

% Glossary
% Creating a glossary with xindy, must be loaded AFTER hyperref
\usepackage[%
xindy,              % sort glossaries
toc                 % add the glossary reference to the toc (table of contents)
]{glossaries}           % create glossaries
\usepackage[%
automake
