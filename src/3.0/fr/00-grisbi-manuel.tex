...
%]{scrbook} 			% TODO change enabledeprecatedfontcommands to sfdefaults=false
%}{
...
\documentclass[%
	...
	12pt,				% default=11pt
	french,				% use in babel, translator and varioref packages options
	toc=listof,			% add a "lof" (list of figures) line at the begining of the toc (table of contents)
	twoside=semi		% same margins recto/verso similar to margins verso, default=twoside
]{scrbook} 				
%}
%\fi
...
\usepackage{babel}			% the language  %TODO: make this customizable ; frenchb in place of french
...
\usepackage[unicode]{hyperref}		% TODO replace \usepackage{url}, used to add an URL and rewrite the "grisbi-manuel-urldef.tex" file
...
%\usepackage{scrlayer-fancyhdr}		% facilities for constructing headers and footers, replace fancyhdr
									% TODO change to scrlayer-scrpage here and in macros.sty
...
%\usepackage{emptypage}		% change by using \cleardoubleemptypage (in koma-scripts) % TODO see to change in macro-3.0.sty
...
\date{%
	Version 3.0 de 2024 (provisoire)
}
%\else
...
% For pdf only; for html, redefined  by an empty command in hva/macros.hva
\frontmatter						% rend les pages numérotées en minuscules romaines et les chapitres non numérotés
...
%\myclearemptydoublepage
% For pdf only; for html, redefined  by an empty command in hva/macros.hva
\mainmatter							% rétablit le comportement à la version attendue et réinitialise le numéro de page.
