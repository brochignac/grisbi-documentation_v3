...
\Attention{}: d'une manière générale, il est déconseillé d'avoir des accents, des espaces ou des caractères spéciaux dans les noms des répertoires et fichiers utilisés par Grisbi. Si c'est le cas, renommez-les maintenant. Par exemple, les espaces peuvent être remplacées par des tirets bas (\_).

Eviter les caractères spéciaux tels que & * % $ £ ° { ! @ qui peuvent être réservés pour des tâches spécifiques selon les systèmes d’exploitation
	Évitez les espaces et les caractères spéciaux tels que : #~! @$^`éèùà&*()’:;<>?.,[]{} «»|%
	Ne pas utiliser de caractères spéciaux (sauf underscore « _ » ) : % $ ! & / \ : ; « » % & # @ etc.
	N’employez pas de caractères spéciaux, des signes de ponctuation ou des accents (#~!
	@$^`éèùà&*()’:;<>?.,[]{} «»|%.).