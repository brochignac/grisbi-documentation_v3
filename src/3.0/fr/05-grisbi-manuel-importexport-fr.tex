...
\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item vous pouvez choisir de ne pas importer les catégories en cochant l'option adéquate. Dans le cas d'import d'un fichier \gls{CSV}, une nouvelle fenêtre vous permet de choisir les paramètres d'import \refimage{importexport-import-CSV-setup-img}:
				\begin{itemize}[label=\textopenbullet]
					...
					\item un bouton à cliquer (en bas) permet de créer une règle d'import que vous pourrez nommer.
				\end{itemize}
	...
	\end{figure}
		\begin{itemize}
			...
			\item \menus{Créer une règle pour cet import}: permet de définir une règle d'importation rapide si le fichier est au format \gls{QIF} ou \gls{OFX} et uniquement si vous ajoutez ou marquez des opérations à un compte. Cette règle est spécifique à chaque compte et peut être nommée. Vous pourrez la retrouver dans la barre d'outils du compte (voir \vref{transactions-functions}),%TODO A CORRIGER et completer pour qif et csv
			%règle que vous pourrez nommer pour la retrouver facilement,
...
Dans le menu \menus{Fichier}, choisissez l'option \menus{Exporter vers un fichier QIF/CSV/\ldots{ }}  (ou utilisez le raccourci-clavier \keys{Ctrl+E}) qui ouvre l'assistant Export des comptes. L'exportation des comptes comporte quatre étapes:
...
\begin{enumerate}
	...
	\item pour chaque compte, définissez le nom du fichier, le répertoire de destination et le format d'exportation, puis validez par le bouton \menus{Suivant} \refimage{importexport-export-img};
	% image centrée
	\begin{figure}[t]
	\begin{center}
