...
\begin{enumerate}
	...
	\end{figure}
		\begin{itemize}
			...
			\item \menus{Marquer les opérations d'un compte}: si des \indexword{opérations orphelines}\index{opération!orpheline} sont trouvées, une fenêtre s'ouvrira en fin d'import pour savoir ce que vous voulez en faire: soit les ajouter, soit les ignorer. Le menu déroulant \menus{Nom du compte}, en dessous, vous permettra de sélectionner le compte dans lequel les opérations seront marquées;
...
\begin{enumerate}
	...
	\item Sélection des comptes et des options \refimage{importexport-export-img}:
		...
		\begin{itemize}
			...
			\begin{itemize}
					\item \menus{Format QIF}: exporte le(s) compte(s) coché(s) au format \gls{QIF}; en plus:
						\begin{itemize}
							\item l'option \menus{Force les dates au format US} enregistre la date au format \frquote{mois/jour/année} (mm/dd/yyyy),
							\item l'option \menus{Force les nombres au format US} utilise le point \frquote{.} comme séparateur de décimale et la virgule \frquote{,} comme séparateur des milliers;
						\end{itemize}
					\item \menus{Format CSV}: exporte le ou les comptes cochés au format \gls{CSV}; en plus des options disponibles au format QIF (ci-dessus):
						...
						\end{itemize}
			\end{itemize}
			\item \menus{Traiter tous les fichiers comme le premier}: ???%TODO: FOLLOWING;
