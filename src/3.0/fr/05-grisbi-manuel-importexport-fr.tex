		...
		\begin{itemize}
			...
			\begin{itemize}
					...
					\item \menus{Format CSV}: exporte le ou les comptes cochés au format \gls{CSV}; en plus des options disponibles au format QIF (ci-dessus):
						...
						\end{itemize}
			\end{itemize}
			\item \menus{Traiter tous les fichiers comme le premier}: cette option exporte le ou les comptes sélectionnés avec le même nom que dans la liste, sans possibilité de modification et sans choix de la destination;
			\item validez par le bouton \menus{Suivant};
		\end{itemize}
	\item pour chaque compte, définissez le nom du fichier, le répertoire de destination et le format d'exportation; l'option \menus{Traiter tous les fichiers comme le premier} shunte cette étape. Puis validez par le bouton \menus{Suivant}
