...
\begin{itemize}
	 \item pour les opérations d'un compte, dans la section \vref{transactions-list-sorts}, \menus{Tris} ;
	 \item pour les opérations planifiées, dans la section \vref{plannedtransactions-list-sorts}, \menus{Tris}.
\end{itemize}
...
\begin{itemize}
	 ...
	 \item double-cliquez sur l'opération recherchée : la liste des opérations du compte concerné s'affiche, dans laquelle l'opération s'affiche sur \colorbox{red}{fond rouge}.	 	 	 
\end{itemize}

%espace pour changement de thème
\vspacepdf{2mm}

Pour plus de détails, reportez-vous respectivement aux sections \vref{thirdparties-list}, \menus{Liste des tiers}, \vref{categories-list}, \menus{Liste des catégories et des sous-catégories}, ou \vref{budgetarylines-list}, \menus{Liste des imputations budgétaires} et suivantes.


\section{Recherche par états\label{search-advanced} }


Si la méthode ci-dessus ne convient pas, une recherche plus élaborée passe par la création d'un état spécifique à cette recherche. Pour cela vous pouvez utiliser l'état pré-formaté \menus{Recherche}, qui vous affichera toutes les informations de toutes les opérations pour tous les comptes de  l'année en cours.

%espace pour changement de thème
\vspacepdf{2mm}
Il vous faudra probablement ajuster le choix dans la date, et préciser d'autres critères, tels un montant (qui doit être affecté d'un signe \frquote{moins} pour une dépense) ou une \indexword{chaîne de caractères}\index{chaîne de caractères} à rechercher. 

 % espace avant Attention ou Note  : 5 mm
\vspacepdf{2mm}
\Note{}: si vous sélectionnez \menus{Toutes les dates}, la génération de l'état risque d'être assez longue. D'une manière générale, moins les critères seront précis, plus cette génération sera longue et plus les résultats seront nombreux.

%espace pour changement de thème
\vspacepdf{2mm}
Une fois l'opération trouvée dans l'état, et si les opérations sont configurées comme \indexword{cliquables}\index{opération !cliquable} (voir le paragraphe \vref{reportscreation-display-transactions-clickable}, \menus{Opérations cliquables}), cliquez sur sa ligne pour afficher directement le détail de l'opération.

%espace pour changement de thème
\vspacepdf{2mm}
Pour savoir comment créer un état, voir le chapitre \vref{reportscreation}, \menus{Création d'un état}.
