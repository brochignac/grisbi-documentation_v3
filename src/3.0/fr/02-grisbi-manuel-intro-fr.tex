...
\begin{itemize}
	...
	\item Les touches \keys{{+}} (\keys{{-}}) dans un champ de date incrémentent (décrémentent) la date
...
\begin{itemize}
	...
	\item Harmonisation dans les champs de date:
		\begin{itemize}
		\item \keys{Ctrl+Entrée \return} dans un champ de date ouvre un calendrier
		\item les touches fléchées sont actives dans un calendrier
		\item \keys{Ctrl+{+}} (\keys{Ctrl+{-}}) dans un champ de date augmente (diminue) la date d'environ une semaine
		\item \keys{Pg.Préc} (\keys{Pg.Suiv}) augmente (diminue) la date d'environ un mois
		\item \keys{Ctrl+Pg.Préc} (\keys{Ctrl+Pg.Suiv}) augmente (diminue) la date d'environ un an
		\end{itemize}	
\end{itemize}
...
\begin{itemize}
	...
	\item Configuration globale en \gls{XML}\index{XML} (merci à Axel Rousseau)
...
\begin{itemize}
	...
	\item Commande d'affichage plein écran par la touche de fonction \keys{F11}
	\item Raccourci-clavier \keys{Ctrl+T} pour l'appel d'une nouvelle opération
	\item Accès direct au manuel de l'utilisateur par le menu \menus{Aide} ou le raccourci-clavier \keys{Ctrl+H}
\end{itemize}
...
Vous pouvez également utiliser les forums de discussions (ou newsgroups) avec un logiciel appelé \frquote{lecteur de nouvelles} (comme Thunderbird par exemple) en renseignant \cmds{listes.grisbi.org} comme nom de serveur de groupes.
...
\Note{}: cette version 3.0 du manuel est une mise à jour de la version 2.0 sauf pour le chapitre \vref{association} \menus{Comptabilité d'association}, qui reste en version 2.0 %TODO to modify when it"ll be updated

\vspacepdf{5mm}			% espace : 5 mm

Ce manuel a été écrit avec le \gls{formateur de texte} \gls{LaTeX}\index{latex@\LaTeX}, et il est disponible sous le \gls{format de fichier} \indexword{\gls{PDF}}\index{PDF} ou \indexword{\gls{HTML}}\index{HTML}, avec illustrations (copies d'écran) dans ces deux formats. 

Il est directement accessible dans le logiciel Grisbi par le menu \menus{Aide - Manuel} de la barre de menus ou par le raccourci-clavier \keys{F1}.

De plus, tous ces différents formats peuvent être téléchargés sur le site de \lang{Sourceforge}\footnote{\urlSourceForgeDocumentation{}}, ainsi que les versions correspondantes du logiciel sur \lang{Sourceforge}\footnote{\urlSourceForge{}} dans les dossiers \frquote{\textsf{grisbi stable}} ou \frquote{\textsf{grisbi unstable}}.
...
\begin{itemize}
	\item \menus{Préambule} explique l'origine du nom donné à ce logiciel;
	\item \menus{Introduction} présente le logiciel, le manuel, leurs auteurs et les contacts;
	\item \menus{Premier démarrage de Grisbi} est le chapitre \emph{indispensable} pour commencer l'utilisation du logiciel;
	\item \menus{Accueil} décrit les éléments principaux de l'interface graphique et leur manipulation à la souris et au clavier (raccourcis);
	\item \menus{Export et Import de comptes} décrit comment échanger des données avec les autres logiciels;
	\item \menus{Gestion des données} présente les options des fichiers de comptes, les sauvegardes et les archives ainsi que leur gestion;
	\item \menus{Gestion des comptes} décrit les propriétés des comptes, leur gestion et les différents types de comptes avec leur utilisation;
	\item \menus{Opérations d'un compte} décrit les manipulations sur les opérations, les champs d'information et de saisie utilisés et leur gestion, la création d'opérations et leur gestion;
	\item \menus{Rapprochement bancaire} détaille la procédure de rapprochement d'un compte et la gestion des rapprochements;		
	\item \menus{Échéancier} décrit la planification des opérations futures et leur manipulation;
	\item \menus{Recherches} fait le point sur les possibilités de recherche de données;
	\item \menus{Tiers}, \menus{Catégories}, \menus{Imputations budgétaires} et \menus{Exercices} décrivent la gestion de ces données;
	\item \menus{Simulation de crédit} décrit les méthodes et la gestion de simulations;
	\item \menus{Budgets prévisionnels} décrit les outils et les procédures de création de budget et de tableaux d'amortissement, ainsi que leur gestion;
	\item \menus{Gestion des cartes bancaires et leurs prévisions} décrit la gestion de ces cartes, notamment celles à débit différé, et les méthodes de prévision;	
	\item \menus{Comptabilité d'association} présente deux introductions pour les trésoriers d'association;
	\item \menus{États} et \menus{Création d'un état} décrivent la gestion et la création des états;
	\item \menus{Configuration de Grisbi} détaille toutes les possibilités de réglage du logiciel;
	\item \menus{Outils de maintenance} donne quelques clés à utiliser en cas d'erreurs ou de bogues.
\end{itemize}
...
\begin{itemize}
	...
	\item les combinaisons de touches sont une série de touches à enfoncer simultanément (à moins d'indication contraire) pour réaliser une fonction unique; elles sont présentées ainsi:
		\newline
		\hspace*{1.5cm}appuyez sur la combinaison de touches \keys{Ctrl+R};
	\item les commandes qui font partie d'une instruction et qui doivent être saisies sont présentées ainsi:
		\newline
		\hspace*{1.5cm}tapez \cmds{grisbi} pour démarrer le programme;
...
% END_TODO 
	\item \Note{}: souligne un point particulier à prendre en compte;
	\item \Attention{}: indique soit un point très important pour la compréhension, soit une erreur à ne pas faire sous peine de dommage important pour vos données;\newline
	\Attention{} indique un point \emph{à respecter impérativement}.
\end{itemize} 
...
\begin{itemize}
	\item pour le format \indexword{\gls{PDF}}\index{PDF}: 
		\begin{itemize}
	...
	\item pour le format \indexword{\gls{HTML}}\index{HTML}:
		\begin{itemize}
